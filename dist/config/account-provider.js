...
function builtinTablePath() {
    ...
    const bundled = process.platform === "win32"
        ? path.join(process.env.LOCALAPPDATA ?? path.join(os.homedir(), "AppData", "Local"), "Programs", "ZCode", "resources", "config", "provider", PROVIDER_CONFIG_NAME)
        : "/Applications/ZCode.app/Contents/Resources/config/provider/zcode-builtin.json";
    return existsSync(bundled) ? bundled : null;
}
function readBuiltinTable() {
    const file = builtinTablePath();
    if (!file)
        return null;
    try {
        return { table: JSON.parse(readFileSync(file, "utf8")), file };
    }
    catch (e) {
        warn(`account-provider: unreadable provider table (${e instanceof Error ? e.message : String(e)})`);
        return null;
    }
}
/**
 ...
 */
function builtinRevision(file, revision) {
 ...
 */
export function accountProviderIdFor(providerId) {
    if (!providerId.startsWith("builtin:"))
        return providerId;
    const rules = readBuiltinTable()?.table.config?.providerConfigRules?.providerRules?.filter((r) => r.config?.access?.type === "zhipu-account") ?? [];
    ...
    const hit = rules.find((r) => r.config?.access?.accountType === family && r.config?.access?.mode === plan);
    return hit?.providerId ?? providerId;
}
/** Reverse map: account-provider id → the config.json id it corresponds to. */
export function configProviderIdFor(providerId) {
    if (!providerId.startsWith("account:"))
        return providerId;
    const rule = readBuiltinTable()?.table.config?.providerConfigRules?.providerRules?.find((r) => r.providerId === providerId);
 ...
 */
function entitledBuiltinProviders() {
    const out = new Set();
    const credsDir = path.dirname(ZCODE_CREDS_PATH);
    // 1) Desktop availability cache (authoritative: the resolved verdict).
    try {
        const cache = JSON.parse(readFileSync(path.join(credsDir, "coding-plan-cache.json"), "utf8"));
    ...
    try {
        ...
        for (const [pid, p] of Object.entries(cfg.provider ?? {})) {
            if (!pid.startsWith("builtin:"))
                continue;
            if (p?.enabled === true && p.options?.apiKey)
                out.add(pid);
        }
    }
    catch {
    ...
    try {
        const setting = JSON.parse(readFileSync(path.join(credsDir, "setting.json"), "utf8"));
 ...
 * store-encrypted.
 */
export function codingPlanRequestAuthFor(accountProviderId) {
    if (!accountProviderId?.startsWith("account:"))
        return null;
    const rule = readBuiltinTable()?.table.config?.providerConfigRules?.providerRules?.find((r) => r.providerId === accountProviderId);
    if (rule?.config?.access?.mode !== "individual-coding-plan")
        return null;
    const legacyId = configProviderIdFor(accountProviderId);
    if (legacyId === accountProviderId)
        return null;
    if (!entitledBuiltinProviders().has(legacyId))
        return null;
    try {
        const cfg = JSON.parse(readFileSync(ZCODE_CREDS_PATH, "utf8"));
...
export function buildAccountProviderConfig() {
    ...
    if (!found)
        return null;
    const rules = found.table.config?.providerConfigRules?.providerRules?.filter((r) => r.config?.access?.type === "zhipu-account") ?? [];
    if (rules.length === 0)
        return null;
 ...
 */
export async function pushAccountProviderConfig(backend, nextId) {
    const payload = buildAccountProviderConfig();
    if (!payload)
        return false;
    try {
        const resp = await backend.request(nextId(), "provider/updateAccountConfig", payload, 10000);
        if (resp.error) {
        ...
        log(`account-provider: pushed ${result.providerCount ?? Object.keys(payload.providers).length} provider(s) (${result.status ?? "ok"})`);
        return true;
    }
    catch (e) {
