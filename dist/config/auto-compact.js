...
export async function maybeAutoCompact(server, cx, acpSid, zcodeSid) {
    ...
    try {
        ...
        try {
            ...
            const resp = await backend.request(server.nextId(), "session/read", { sessionId: zcodeSid, messageLimit: 1 }, 5000);
            if (resp.error)
                return;
 ...
 */
export function runAutoCompactDetached(server, cx, acpSid, zcodeSid) {
 ...
 */
export async function waitForAutoCompactIdle(server, zcodeSid, timeoutMs = AUTO_COMPACT_SETTLE_MS) {
