...
export function providerSelectable(pid, p) {
    ...
    if (p.options?.apiKey)
        return true;
    if (p.options?.apiKeyRequired === false)
        return true;
 ...
 */
export function formatModelValue(providerId, modelId) {
    return `${providerId}\\${modelId}`;
}
function collidingModelIds(models) {
...
export async function buildModes(server, zcodeSid) {
    let currentMode = "yolo";
    if (zcodeSid !== null) {
        try {
            const read = await sessionRead(server, zcodeSid);
            const settings = (read.settings ?? {});
            const modeSet = settings.mode ?? {};
...
export function orderThoughtVariants(variants) {
...
export async function buildConfigOptions(server, zcodeSid, receiverRoot) {
    ...
    if (zcodeSid === null) {
        ...
        if (cur) {
            ...
            try {
                const cfg = readConfig();
                const m = cfg.provider?.[cur.providerId]?.models?.[cur.modelId];
    ...
    if (zcodeSid !== null) {
        try {
            ...
            currentThought =
                tlSet.current ?? tlSet.defaultLevel ?? currentThought;
            const tlAvail = tlSet.available ?? [];
            if (tlAvail.length > 0) {
    ...
    const omitMode = server.isPaseoClient() ||
