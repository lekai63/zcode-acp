...
const DYNAMIC_WORKFLOW_MODES = ["disabled", "onDemand", "alwaysOn"];
/** The fail-closed verdict every failure path collapses to. */
const GATE_DISABLED = { mode: "unknown", enabled: false, source: "default" };
 ...
 */
export async function resolveWorkflowGate(fetchImpl = globalThis.fetch) {
    try {
        ...
        if (typeof mode !== "string" || !DYNAMIC_WORKFLOW_MODES.includes(mode)) {
            return GATE_DISABLED;
        }
        return {
            mode: mode,
 ...
 */
const settledGates = new WeakMap();
/**
 ...
 */
export function captureGate(promise) {
    void promise.then((gate) => settledGates.set(promise, gate), () => settledGates.set(promise, GATE_DISABLED));
    return promise;
}
/**
 ...
 */
export function rememberGate(promise, value) {
 ...
 * went through it.
 */
export function workflowGateNow(server) {
 ...
 */
export function filterWorkflowCommands(server, commands) {
 ...
 * an older backend.
 */
export async function pushDynamicWorkflowPolicy(backend, nextId, cwd) {
    try {
        const resp = await backend.request(nextId(), "workspace/updateDynamicWorkflowPolicy", { workspace: { workspacePath: cwd, workspaceKey: cwd }, enabled: true }, POLICY_PUSH_TIMEOUT_MS);
        if (resp.error) {
