...
export async function applyModelSwitch(server, zcodeSid, value) {
    ...
    const model = { providerId: registryProviderId, modelId };
    ...
    const modern = await backend.request(server.nextId(), "session/setModel", { sessionId: zcodeSid, model, persistAsWorkspaceLastUsed: false }, 15000);
    if (!modern.error) {
    ...
    if (runtimeModel !== null) {
        const legacy = await backend.request(server.nextId(), "session/setModel", {
            sessionId: zcodeSid,
        ...
        warn(`runtime-model: switch failed (modern: ${modern.error.message}; legacy: ${legacy.error.message})`);
        return false;
    }
 ...
 */
function resolveDefaultReasoningLevel(server, zcodeSid, providerId, modelId) {
    const cached = server.modelAvailability.get(zcodeSid) ?? [];
    const hit = cached.find((a) => a.providerId === providerId && a.modelId === modelId);
    if (hit?.defaultLevel)
        return hit.defaultLevel;
    if (hit)
        return null; // present but level-less — omit options
    ...
    try {
        ...
        if (reasoning && reasoning.enabled !== false) {
            if (reasoning.defaultVariant)
                return reasoning.defaultVariant;
            if (reasoning.variants?.length)
                return reasoning.variants[0];
        }
    }
    catch {
