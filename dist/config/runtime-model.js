...
import { findProviderConfig, formatModelValue, isBuiltinProvider, loadAllModels, parseModelValue, personalModelSpec, } from "./options.js";
...
export async function applyModelSwitch(server, zcodeSid, value) {
    ...
    const resp = await backend.request(server.nextId(), "session/setModel", { sessionId: zcodeSid, model, persistAsWorkspaceLastUsed: false }, 15000);
    if (resp.error) {
