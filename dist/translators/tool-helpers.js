 ...
 */
export function summarizeToolInput(_toolName, inp) {
    if (inp == null)
        return "";
 ...
 */
export function renderToolOutput(output) {
    if (output == null)
        return "";
    if (typeof output === "string")
        return output.slice(0, OUTPUT_MAX);
    if (typeof output === "object") {
        if (!Array.isArray(output)) {
            const obj = output;
            const content = obj["content"];
            if (typeof content === "string" && content)
                return content.slice(0, OUTPUT_MAX);
            if (obj["success"] === false) {
                const err = String(obj["error"] ?? obj["message"] ?? "");
                if (err)
                    return `[failed] ${err.slice(0, OUTPUT_MAX)}`;
            }
            try {
 ...
 */
export function extractExitCode(resultPayload, isError = false) {
 ...
 */
export function buildResultContent(toolName, resultPayload, isError = false) {
    let text = "";
    if (resultPayload && typeof resultPayload === "object" && !Array.isArray(resultPayload)) {
        const obj = resultPayload;
        const c = obj["content"];
 ...
 */
export function buildDiffContent(display) {
 ...
 */
export function extractLocations(toolName, inp, display) {
...
const USAGE_RE = /<usage>\s*subagent_tokens:\s*(\d+)\s*tool_uses:\s*(\d+)\s*duration_ms:\s*(\d+)\s*<\/usage>/;
 ...
 */
export function parseSubagentMetadata(rawContent) {
    let text;
    ...
    if (!text.includes("agentId:") && !text.includes("<usage>"))
        return null;
    const meta = {};
    ...
    if (meta.agentId === undefined && meta.tokens === undefined)
        return null;
    return meta;
}
 ...
 */
export function isTransientTurnError(error) {
    if (!error || typeof error !== "object" || Array.isArray(error))
        return false;
    const e = error;
    const cause = e["cause"];
...
function matchesTransient(node) {
    ...
    if (code && TRANSIENT_CAUSE_CODES.has(code))
        return true;
    const message = String(n["message"] ?? n["detail"] ?? "").toLowerCase();
    if (message && TRANSIENT_MSG_KEYWORDS.some((kw) => message.includes(kw)))
        return true;
    return false;
}
/**
 ...
 */
export function isBackendLostError(error) {
    if (!error || typeof error !== "object" || Array.isArray(error))
        return false;
    const e = error;
    const cause = e["cause"];
    if (matchesBackendLost(cause))
        return true;
    if (cause !== undefined) {
        const inner = cause["cause"];
        if (matchesBackendLost(inner))
            return true;
    }
    return cause === undefined && matchesBackendLost(e);
}
function matchesBackendLost(node) {
    if (!node || typeof node !== "object" || Array.isArray(node))
        return false;
    const n = node;
    const code = String(n["code"] ?? "").trim();
    if (code && BACKEND_LOST_CODES.has(code))
        return true;
    const message = String(n["message"] ?? n["detail"] ?? "").toLowerCase();
    if (message && BACKEND_LOST_MSG_KEYWORDS.some((kw) => message.includes(kw)))
        return true;
    return false;
}
