...
function buildCheckpointTarget(params) {
    if (params.checkpointId)
        return { kind: "checkpoint", checkpointId: params.checkpointId };
    if (params.target)
 ...
 */
async function resolveSidOrThrow(server, params) {
    return ensureRealSession(server, params.sessionId);
}
/** session/fork → zcode session/fork: branch a new session from a checkpoint. */
export async function fork(server, params) {
    const zcodeSid = await resolveSidOrThrow(server, params);
    const backend = await server.ensureBackend();
    const resp = await backend.request(server.nextId(), "session/fork", { sessionId: zcodeSid, target: buildCheckpointTarget(params) }, 15000);
    if (resp.error)
        throw new Error(`fork failed: ${resp.error.message}`);
...
export async function goal(server, params) {
    const zcodeSid = await resolveSidOrThrow(server, params);
    const action = params.action ?? "show";
    const zcParams = { sessionId: zcodeSid, action };
    if ((action === "set" || action === "replace") && params.objective !== undefined) {
        zcParams.objective = params.objective;
    }
    const resp = await (await server.ensureBackend()).request(server.nextId(), "session/goal", zcParams, 15000);
    if (resp.error)
        throw new Error(`goal failed: ${resp.error.message}`);
 ...
 */
export async function compact(server, params, cx) {
...
async function runCompact(server, params, cx, acpSid, zcodeSid) {
    ...
    // expectedRevision?}).
    const zcParams = { sessionId: zcodeSid };
    const instructions = typeof params.instructions === "string" ? params.instructions.trim() : "";
    if (instructions)
        zcParams.instructions = instructions;
    const startedAt = Date.now();
    const resp = await (await server.ensureBackend()).request(server.nextId(), "session/compact", zcParams, 30000);
    if (resp.error)
        throw new Error(`compact failed: ${resp.error.message}`);
    const ack = (resp.result ?? {}).compact?.state;
...
export async function cancelBackgroundTask(server, params) {
    const zcodeSid = await resolveSidOrThrow(server, params);
    const taskId = String(params.taskId ?? "");
    if (!taskId)
        throw new Error("cancelBackgroundTask requires taskId");
    const resp = await (await server.ensureBackend()).request(server.nextId(), "session/cancelBackgroundTask", { sessionId: zcodeSid, taskId }, 15000);
    if (resp.error)
        throw new Error(`cancelBackgroundTask failed: ${resp.error.message}`);
    ...
    if (listener)
        void listener.markCancelled(taskId);
    log(`session/cancelBackgroundTask → cancelled=${resp.result?.cancelled}`);
    return (resp.result ?? {});
}
...
export async function setThoughtLevel(server, params, cx) {
    ...
    const zcParams = { sessionId: zcodeSid };
    if (params.thoughtLevel !== undefined)
        zcParams.thoughtLevel = params.thoughtLevel;
    const resp = await (await server.ensureBackend()).request(server.nextId(), "session/setThoughtLevel", zcParams, 15000);
    if (resp.error)
...
export async function setModel(server, params, cx) {
    ...
    if (!modelId)
        throw new Error("setModel requires modelId");
...
export async function setMode(server, params, cx) {
    const acpSid = params.sessionId;
    const zcodeSid = await resolveSidOrThrow(server, params);
    ...
    if (!mode)
        throw new Error("setMode requires mode");
    const resp = await (await server.ensureBackend()).request(server.nextId(), "session/setMode", { sessionId: zcodeSid, mode }, 15000);
    if (resp.error)
 ...
 */
export async function waitForTurnIdle(server, zcodeSid, timeoutMs, probeMethod, expectLock, graceMs = 30_000) {
    const backend = await server.ensureBackend();
    const t0 = Date.now();
    let lockSeen = !expectLock;
    let probeCount = 0;
    log(`  [probe] start waitForTurnIdle timeout=${Math.round(timeoutMs / 1000)}s expectLock=${expectLock} probeMethod=${probeMethod}`);
    while (Date.now() - t0 < timeoutMs) {
        probeCount++;
        const elapsed = Math.round((Date.now() - t0) / 100) / 10;
        const resp = await backend.request(server.nextId(), probeMethod, { sessionId: zcodeSid, action: "show" }, 10000);
        ...
        if (errMsg.includes("prompt is running") || errMsg.includes("already running")) {
            log(`  [probe] #${probeCount} @${elapsed}s: LOCK HELD ("${errMsg.slice(0, 60)}") → lockSeen=true`);
            lockSeen = true;
            await sleep(2000);
            continue;
        }
        ...
        if (resp.error) {
            ...
            if (lockSeen) {
                log(`  [probe] #${probeCount} @${elapsed}s: NON-LOCK error after lock → released (err="${errMsg.slice(0, 50)}")`);
                return true;
            }
            if (Date.now() - t0 >= graceMs) {
                log(`  [probe] #${probeCount} @${elapsed}s: NON-LOCK error, grace expired → released (err="${errMsg.slice(0, 50)}")`);
                return true;
            }
            log(`  [probe] #${probeCount} @${elapsed}s: NON-LOCK error, lockSeen=false → wait for lock (err="${errMsg.slice(0, 50)}")`);
            await sleep(500);
            continue;
        }
        if (lockSeen) {
        ...
        log(`  [probe] #${probeCount} @${elapsed}s: probe success, lockSeen=false → wait for lock (still in startup window)`);
        await sleep(500);
    }
