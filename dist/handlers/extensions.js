...
export async function compact(server, params, cx) {
    ...
    return { ...(resp.result ?? {}), __lockTimeout: !released };
}
...
export async function cancelBackgroundTask(server, params) {
    ...
    const resp = await server
        .ensureBackend()
        .request(server.nextId(), "session/cancelBackgroundTask", { sessionId: zcodeSid, taskId }, 15000);
    if (resp.error)
...
export async function updateRuntimeModelConfig(server, params, cx) {
    ...
    if (!runtimeModel)
        throw new Error("updateRuntimeModelConfig requires runtimeModel");
    const zcParams = { sessionId: zcodeSid, runtimeModel };
