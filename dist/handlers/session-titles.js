...
export class SessionTitleListener {
    server;
    zcodeSid;
    ...
    handleEvent(event) {
        ...
        void this.onTitleUpdated(event.payload).catch((e) => log(`SessionTitleListener: title update failed: ${e instanceof Error ? e.message : String(e)}`));
    }
    async onTitleUpdated(p) {
        const source = p.source;
        if (source !== "generated" && source !== "custom")
            return;
        ...
        if (!primary)
            return; // no ACP alias attached yet — nothing to update
            ...
            try {
                if (await isTitleOverridden(this.zcodeSid)) {
                    ...
                    for (const sid of aliases)
                        this.server.titleUserSetBy.add(sid);
                }
            }
            catch {
        ...
        if (aliases.every((sid) => this.server.sessionTitles.get(sid) === title))
            return;
        for (const sid of aliases) {
        ...
        void updateSessionTitle(this.zcodeSid, title, title).catch((e) => log(`SessionTitleListener: tasks-index persist failed (non-fatal): ${e instanceof Error ? e.message : String(e)}`));
        ...
        for (const sid of aliases) {
            ...
            }).catch((e) => log(`SessionTitleListener: session_info_update broadcast failed: ${e instanceof Error ? e.message : String(e)}`));
        }
