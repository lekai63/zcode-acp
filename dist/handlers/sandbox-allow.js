...
import { appendSandboxAllow, appendSandboxDeny, collectSandboxWorkspaces, readSandboxConfig, resolveReal, } from "../backend/sandbox.js";
 ...
 */
export function extractSandboxDenial(text) {
    if (!text.toLowerCase().includes(EPERM.toLowerCase()))
        return null;
    const mkdirMatch = /mkdir: ((?:\/|\.{1,2}\/)[^\s"\\:]+): /i.exec(text);
    if (mkdirMatch)
        return { path: mkdirMatch[1], isMkdir: true };
    ...
    if (nodeForm)
        return { path: nodeForm[2], isMkdir: nodeForm[1].toLowerCase() === "mkdir" };
...
export class SandboxRestartBatcher {
    ...
    grants = new Map();
    timer = null;
    constructor(flush, batchMs = SANDBOX_RESTART_BATCH_MS) {
        this.flush = flush;
        this.batchMs = batchMs;
    }
    /** Add one approved path for a session; the first add arms the window. */
    add(acpSid, grantedReal) {
        const list = this.grants.get(acpSid) ?? [];
        if (!list.includes(grantedReal))
            list.push(grantedReal);
 ...
 */
export function flushSandboxGrants(target, grants) {
    const m = messages();
    ...
    for (const t of target.pendingTurns.values()) {
        ...
        if (t.goalLoop && !t.cancelled)
            t.sandboxRestart = true;
    }
    ...
    // "continue" round.
    const liveZcode = new Set();
    for (const t of target.pendingTurns.values()) {
        if (!t.goalLoop)
            liveZcode.add(t.zcodeSid);
    }
    ...
    dying
        .close()
        .catch((e) => warn(`sandbox: backend kill failed: ${e instanceof Error ? e.message : String(e)}`));
}
/**
 ...
 */
export function extractPermDeniedPath(text) {
 ...
 */
export async function handleSandboxDenial(server, cx, acpSid, denial, toolCallId) {
    ...
    const wsRoot = cwd;
    const targetReal = resolveReal(path.isAbsolute(raw) ? raw : path.resolve(cwd ?? process.cwd(), raw));
    ...
    const options = [
        ...
        { optionId: "sandbox_allow_once", kind: "allow_once", name: m.sandboxOptionAllowOnce },
        {
    ...
    try {
        ...
        const resp = (await withTimeout(cx.request("session/request_permission", {
            sessionId: acpSid,
            toolCall: {
                toolCallId,
        ...
        if (oid === "sandbox_allow_always")
            decision = "always";
        else if (oid === "sandbox_allow_once")
            decision = "once";
        else if (oid === "sandbox_reject_always")
            decision = "reject_always";
        else if (oid === "sandbox_reject_once")
        ...
        warn(`sandbox: allow ask for ${targetReal} failed: ${e instanceof Error ? e.message : String(e)}`);
        return;
    }
    ...
    if (decision === "reject_always") {
        ...
        await sendTextChunk(cx, acpSid, persisted
            ? m.sandboxRejectAlwaysPersisted(targetReal)
            : m.sandboxRejectAlwaysUnpersisted(targetReal), randomUUID());
        return;
    }
    ...
    if (decision === "always") {
        ...
        if (wsRoot) {
            if (!appendSandboxAllow(wsRoot, targetReal))
