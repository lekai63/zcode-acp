...
export class BackgroundTaskListener {
    ...
    async onTaskStatus(p) {
        ...
        if (p.taskKind === "workflow") {
            ...
            if (tracked?.foldedIntoToolCard) {
                if (acpStatus !== "in_progress")
                    stopWorkflowRunPoller(taskId);
            ...
            if (!tracked) {
                ...
                if (cardId) {
                    ...
                    log(`  [bg] workflow run ${taskId.slice(-12)} → progress folded into card ${cardId.slice(-12)}`);
                    return;
                }
            }
            if (acpStatus !== "in_progress")
    ...
    async emitShutdownRecords() {
        for (const [taskId, task] of [...this.tasks]) {
            if (task.lastStatus === "completed" || task.lastStatus === "failed")
                continue;
            if (task.reusesLaunchCard && task.sourceToolCallId) {
                await this.server.notifyByZcodeSid(this.zcodeSid, {
                    sessionUpdate: "tool_call_update",
