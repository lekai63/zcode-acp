 ...
 */
export function mapSubagentStatus(raw, phase) {
...
export class SubagentTracker {
    ...
    handleEvent(event) {
        try {
            ...
            switch (type) {
                case "tool.updated":
                    if (readString(payload["source"]) === "subagent")
                        this.onMirroredTool(payload);
                    else
                        this.onParentTool(payload);
                    return;
                case "state.updated":
    ...
    onMirroredTool(payload) {
        ...
        if (agent.description && !agent.title)
            agent.title = agent.description;
        const kind = readString(payload["kind"]) ?? "";
        const toolName = readString(payload["toolName"]);
        const key = `${agentId}:${childToolCallId ?? ""}`;
        if (toolName && childToolCallId)
            this.childToolNames.set(key, toolName);
        ...
        if (kind === "scheduled" || kind === "started") {
            ...
            if (summary)
                this.childToolSummaries.set(key, summary);
        ...
        if (kind === "result") {
            ...
            this.upsertLine(agent, lineKey, `[${name || "tool"}]${summary ? ` ${summary}` : ""} · ${failed ? "failed" : "done"}`);
            this.requestEmit(agent);
            return;
        }
        if (kind === "error") {
            const summary = this.childToolSummaries.get(key) ?? summarizeToolInput(name, payload["input"]);
            this.upsertLine(agent, lineKey, `[${name || "tool"}]${summary ? ` ${summary}` : ""} · failed`);
            this.requestEmit(agent);
        }
    }
     ...
     */
    onParentTool(payload) {
        const toolName = readString(payload["toolName"]) ?? "";
        const isDispatchName = toolName === "Agent" || toolName === "Task";
        const callId = readString(payload["toolCallId"]);
        const kind = readString(payload["kind"]);
        if (isDispatchName && callId)
            this.dispatchCallIds.add(callId);
        if (kind !== "result" && kind !== "error") {
            if (isDispatchName && (kind === "scheduled" || kind === "started"))
                this.scheduleRefresh();
            return;
        }
        if (!callId)
        ...
        if (meta.agentId) {
            ...
            if (meta.tokens !== undefined)
                agent.tokens = meta.tokens;
            if (meta.toolUses !== undefined)
                agent.toolUses = meta.toolUses;
            if (meta.durationMs !== undefined)
                agent.durationMs = meta.durationMs;
            this.requestEmit(agent);
        }
    ...
    onLifecycle(payload, phase) {
        ...
        if (phase === "spawned") {
            ...
            this.appendLine(agent, `lifecycle:spawned:${agentId}`, `spawned${agent.agentType ? ` · ${agent.agentType}` : ""}${agent.background ? " · background" : ""}`);
            this.requestEmit(agent);
            this.scheduleRefresh();
        }
        else {
            void this.finalize(agent, mapSubagentStatus(payload["status"], "stopped"), readString(payload["error"]));
        }
    }
    onSubagentMessage(payload) {
        const agentId = readString(payload["agentId"]);
        if (!agentId)
            return;
        const agent = this.upsert(agentId, {
    ...
    onStateUpdated(payload) {
        ...
        for (const item of running)
            this.mergeDirectoryItem(item, "running");
    }
    // ---------- protocol directory ----------
    scheduleRefresh(delayMs = REFRESH_DEBOUNCE_MS) {
    ...
    async refreshFromProtocol() {
        ...
        try {
            const resp = await backend.request(this.server.nextId(), "session/subagents", { sessionId: this.zcodeSid, endedLimit: 20 }, SUBAGENT_REFRESH_TIMEOUT_MS);
            if (resp.error) {
            ...
            if (Array.isArray(ended))
                for (const item of ended)
                    this.mergeDirectoryItem(item, "ended");
        }
        catch (e) {
     ...
     */
    mergeDirectoryItem(item, bucket) {
        ...
        if (bucket === "ended") {
            ...
            if (agent.status === "running")
                void this.finalize(agent, status);
            return;
        }
    ...
    upsert(agentId, patch) {
        ...
        if (patch.agentType)
            agent.agentType = patch.agentType;
        if (patch.title)
            agent.title = patch.title;
        if (patch.description && !agent.description)
            agent.description = patch.description;
        if (patch.childSessionId)
            agent.childSessionId = patch.childSessionId;
        ...
        if (patch.background !== undefined && patch.background)
            agent.background = true;
        if (patch.startedAt !== undefined)
            agent.startedAt ??= patch.startedAt;
        if (patch.status && agent.status === "running")
            agent.status = patch.status;
        return agent;
    }
    async finalize(agent, status, detail) {
        if (agent.status !== "running")
            return;
        agent.status = status === "running" ? "completed" : status;
        agent.endedAt ??= Date.now();
        if (detail && status === "failed")
            agent.errorMessage = detail;
        this.appendLine(agent, `lifecycle:stopped:${agent.agentId}`, `stopped · ${agent.status}${detail ? ` · ${detail}` : ""}`);
        // Terminal transitions must not wait out the throttle window.
        await this.emitNow(agent);
    }
    // ---------- log + emission ----------
    appendLine(agent, key, raw) {
        const text = normalizeLine(raw);
        if (!text || agent.lineKeys.has(key))
            return;
        ...
        while (agent.lines.length > MAX_LOG_LINES) {
            ...
            if (removed)
                agent.lineKeys.delete(removed.key);
        }
    }
    /**
    ...
    async emitNow(agent) {
        ...
        agent.revision += 1;
        try {
            await this.server.notifyVendorByZcodeSid(this.zcodeSid, SUBAGENT_NOTIFICATION_METHOD, this.payloadFor(agent));
        }
        catch (e) {
 ...
 */
export function parseSubagentResultMeta(rawResult) {
    const meta = {};
    const text = subagentResultText(rawResult);
    if (text) {
        const agentId = text.match(/agentId:\s*(agent_[A-Za-z0-9-]+)/);
        if (agentId)
            meta.agentId = agentId[1];
        const usage = text.match(/<usage>\s*subagent_tokens:\s*(\d+)\s*tool_uses:\s*(\d+)\s*duration_ms:\s*(\d+)\s*<\/usage>/);
        if (usage) {
 ...
 */
function subagentResultText(rawResult) {
    ...
    if (output && typeof output === "object")
        return subagentResultText(output);
    try {
