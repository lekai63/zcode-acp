...
import { askSessionPick } from "./server-requests.js";
...
function resumeLabel(title, updatedAt) {
...
function knownCommandSet() {
    if (!knownCommands) {
        const names = [
            ...
            ...loadSkillCommands().flatMap((c) => c.name.startsWith("$") ? [c.name, c.name.slice(1)] : [c.name]),
        ];
 ...
 */
async function autoFlow(server, acpSid, zcodeSid, arg, label, opts = {}) {
    ...
    if (sub === "pause") {
        ...
        if (!live)
            return messages().goalPaused("not running");
        live.pause();
    ...
    if (sub === "resume") {
        ...
        if (live) {
            ...
            if (!prior) {
                throw new RequestError(-32602, label === "goal" ? messages().slashErrGoalArg : messages().slashErrAutoArg);
            }
...
export async function handleSlashCommand(server, cx, acpSid, zcodeSid, text, client) {
    ...
    try {
        switch (cmd) {
            ...
            case "fork": {
                const result = (await fork(server, { sessionId: acpSid }));
            ...
            case "resume": {
                ...
                if (arg && !target)
                    throw new RequestError(-32602, messages().slashErrResumeArg(arg));
                let chosen = target ?? null;
                if (!chosen) {
                    if (resumable.length === 0)
                        return ok(messages().slashResumeNone);
                    const items = resumable.slice(0, RESUME_PICK_LIMIT).map((s) => ({
            ...
            case "model": {
                if (!arg)
                    throw new RequestError(-32602, messages().slashErrModelArg);
            ...
            case "thought": {
                if (!arg)
                    throw new RequestError(-32602, messages().slashErrArg(cmd));
