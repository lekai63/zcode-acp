...
function turnStarts(messages) {
    ...
    messages.forEach((m, i) => {
        if (m.info?.role === "user" && i > 0 && !isHiddenUser(m))
            starts.push(i);
    });
    return starts;
}
...
function dedupeMessages(messages) {
    ...
    if (firstIndex.size === messages.length)
        return messages;
    return messages
        .map((m, i) => {
        const id = m.info?.id;
...
function stripSystemReminders(text) {
    return text
        ...
        .replace(/The (?:TodoWrite|Read) tool hasn't been used recently\.[\s\S]*?This is just a gentle reminder - ignore if not applicable\./g, "")
        .replace(/Here (?:are|is)[^\n]*todo list:\s*\n\s*\n\[[\s\S]*?\](?=\n|$)/g, "")
        .trim();
}
/**
...
function notificationTitle(text) {
 ...
 */
function collapseUserText(text, semantics, summary) {
    if (semantics?.kind === "compact_summary") {
        const raw = typeof summary?.title === "string" && summary.title
            ? summary.title
 ...
 */
export async function replayMessages(cx, acpSid, messages, opts = {}) {
    ...
    for (const [mi, m] of messages.entries()) {
        ...
        for (const p of m.parts ?? []) {
            ...
            if (ptype === "text") {
                ...
                const collapse = role === "user" ? collapseUserText(text, info.semantics, info.summary) : null;
                if (collapse) {
                    if (collapse.kind === "tool-transcript" && !toolsKept(mi))
                        continue;
                ...
                if (st.input !== undefined && st.input !== null) {
                    const inputText = typeof st.input === "string" ? st.input : JSON.stringify(st.input, null, 2);
                    if (inputText) {
