 ...
 */
export function echoUserPromptToOthers(server, prompter, params) {
    ...
    void enqueueSessionSend(params.sessionId, async () => {
        ...
        const results = await Promise.allSettled(server.sessionAliases(params.sessionId).map((sid) => server.clients.notifyOthers(prompter, "session/update", {
            sessionId: sid,
            update: {
        ...
        for (const r of results) {
            if (r.status === "rejected") {
                warn(`user-prompt echo failed (sid=${params.sessionId}): ` +
 ...
 */
export function sendSessionUpdateToOthers(server, source, sessionId, update) {
    void enqueueSessionSend(sessionId, async () => {
        const results = await Promise.allSettled(server
            .sessionAliases(sessionId)
            .map((sid) => server.clients.notifyOthers(source, "session/update", { sessionId: sid, update })));
        for (const r of results) {
            if (r.status === "rejected") {
                warn(`update broadcast failed (sid=${sessionId}): ` +
                    `${r.reason instanceof Error ? r.reason.message : String(r.reason)}`);
            }
        }
    });
...
export function sendAvailableCommands(cx, sessionId, commands) {
    return sendSessionUpdate(cx, sessionId, {
 ...
 */
export function skillPrefixForClient(name) {
    if (name === null || name.toLowerCase().includes("martty"))
        return "";
    return "$";
}
/**
 ...
 */
export function sendAvailableCommandsPerClient(registry, sessionId, commands) {
 ...
 */
const activeDeferredTimeouts = new Map();
/**
 ...
 */
export function sendAvailableCommandsDeferred(registry, sessionId, commands) {
    ...
    for (const delay of [50, 300, 1000]) {
        const t = setTimeout(() => {
            sendAvailableCommandsPerClient(registry, sessionId, commands)
                .catch((e) => {
                warn(`available_commands_update failed (sid=${sessionId}, delay=${delay}): ` +
 ...
 */
export async function emitSessionTurnState(server, acpSid, running, cx) {
