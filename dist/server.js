 ...
 */
import { builtinProviderEnv, loadZcodeCredentials, mergeEnvWithCreds, resolveZcodeCommand, zcodeDataBaseDirEnv, ZcodeBackend, } from "./backend/index.js";
import { armSandboxArgv, collectSandboxWorkspaces, sandboxActive } from "./backend/sandbox.js";
import { captureGate, pushDynamicWorkflowPolicy, resolveWorkflowGate, } from "./config/workflow-gate.js";
...
export class ZcodeAcpServer {
     ...
     * paths.
     */
    sandboxRestartBatcher = new SandboxRestartBatcher((grants) => flushSandboxGrants(this, grants));
    /**
     ...
     */
    bootResumeTriggerConnection = null;
    /**
     ...
     */
    noteDispatchedToolCall(backendCallId, acpCallId) {
        ...
        if (this.dispatchedToolCalls.size > 2048) {
            ...
            if (oldest !== undefined)
                this.dispatchedToolCalls.delete(oldest);
        }
    }
    /**
    ...
    async ensureBackend() {
        ...
        const workflowGate = captureGate(resolveWorkflowGate().catch(() => ({
            mode: "unknown",
            enabled: false,
            source: "default",
        })));
        ...
        backend.providerRuntimeHeadersResponder = (id, params) => {
            setImmediate(() => {
                try {
                    answerProviderRuntimeHeaders(backend, id, "interaction/requestProviderRuntimeHeaders", params);
                }
                catch (e) {
                    warn(`provider runtime headers responder threw, declining: ` +
     ...
     */
    cancelAllPendingTurns(skipGoalLoop = false) {
        for (const turn of this.pendingTurns.values()) {
            if (skipGoalLoop && turn.goalLoop)
                continue;
            turn.cancelled = true;
            turn.stopSent = true;
        }
     ...
     */
    async ensureBackgroundListener(zcodeSid) {
        ...
        await this.ensureSubagentTracker(zcodeSid);
     ...
     */
    async ensureSubagentTracker(zcodeSid) {
        const backend = await this.ensureBackend();
    ...
    async initialize(params, client) {
        const clientInfo = params.clientInfo ?? null;
        this.clientName = clientInfo?.name ?? null;
        if (client)
            this.clients.nameConnection(client, this.clientName ?? "");
        if ((this.clientName ?? "").toLowerCase().includes("martty")) {
            this.marttyClientSeen = true;
            const root = clientConnectionRoot(client);
            if (root !== undefined)
                this.marttyConnectionRoots.add(root);
        }
        if ((this.clientName ?? "").toLowerCase().includes("paseo")) {
            const root = clientConnectionRoot(client);
            if (root !== undefined)
                this.paseoConnectionRoots.add(root);
        }
