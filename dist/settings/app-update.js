...
const CHANNEL_VALUES = { stable: 1, preview: 3 };
/** Node platform → the manifest's spelling (manifestUpdateProvider.ts:57-68). */
export function releasePlatform(platform, arch) {
    const os = platform === "win32" ? "windows" : platform === "darwin" ? "darwin" : platform;
    const cpu = arch === "arm64" ? "aarch64" : arch === "x64" ? "x86_64" : arch === "ia32" ? "x86" : arch;
    return `${os}-${cpu}`;
}
/**
 ...
 * the Linux prefixes.
 */
export function appBundlePath(env = process.env) {
    ...
    const candidates = process.platform === "win32"
        ? [
            path.join(env.LOCALAPPDATA ?? path.join(home, "AppData", "Local"), "Programs", "ZCode", "ZCode.exe"),
        ]
 ...
 */
export async function installedAppVersion(appPath, env = process.env, platform = updatePlatform) {
    if (platform !== "darwin")
        return env.ZCODE_APP_VERSION?.trim() || null;
...
export function parseManifest(raw) {
    ...
    const flushBlock = () => {
        ...
        const text = block.lines
            ...
            .join("\n")
            .replace(/\n+$/u, "");
        if (block.target === "root")
            root[block.key] = text;
        else if (block.target === "file" && file)
            file[block.key] = text;
        else if (block.target === "locale" && locale)
            locale[block.key] = text;
        block = null;
    };
    for (const line of lines) {
        ...
        if (context === "locale") {
            ...
            if (locale)
                locale[key] = unquote(value);
        }
    }
    flushBlock();
    if (!root["version"])
        throw new UpstreamError("manifest_missing_version");
    return {
 ...
 */
export function pickReleaseFile(manifest) {
 ...
 */
export async function fetchReleaseManifest(channel, env = process.env, fetchImpl = manifestNetwork) {
    ...
    if (!res.ok)
        throw new UpstreamError(`manifest_http_${res.status}`);
 ...
 */
let manifestNetwork = fetch;
/** Override the update network. Pass `fetch` to restore the default. */
export function setManifestNetworkForTest(impl) {
 ...
 */
let updatePlatform = process.platform;
/** Override the platform the update flows run as (test seam). */
export function setAppUpdatePlatformForTest(platform) {
    updatePlatform = platform;
}
/** The platform the update flows run as. */
function currentPlatform(overridden) {
...
export async function checkForAppUpdate(options = {}) {
    ...
    const updateAvailable = currentVersion !== null && isNewerVersion(manifest.version, currentVersion);
    return {
        updateAvailable,
        currentVersion,
 ...
 */
export async function downloadRelease(file, options = {}) {
    ...
    if (!res.ok || !res.body)
        throw new UpstreamError(`download_http_${res.status}`);
    ...
    if (!verified)
        throw new UpstreamError("download_checksum_mismatch");
 ...
 */
let extractCommand = defaultExtract;
...
export function setExtractForTest(impl) {
    extractCommand = impl;
}
/** Restore the platform's own unzip (test seam). */
export function resetExtractForTest() {
...
async function extractZip(zipPath, destDir) {
    ...
    if (!info.isDirectory())
        throw new Error("archive_app_bundle_is_not_a_directory");
    return bundle;
}
/**
 ...
 */
function canWriteInstallLocation(appPath) {
 ...
 */
let renameFile = rename;
/** Override the rename used by the installer (test seam). */
export function setInstallRenameForTest(impl) {
    renameFile = impl;
}
export function startAppUpdate(file, version, options = {}) {
    if (inFlight)
        return inFlight;
    const env = options.env ?? process.env;
    const platform = currentPlatform(options.platform);
        ...
        try {
            ...
            try {
                ...
                if (!canWriteInstallLocation(appPath)) {
                    ...
                    warn(`app-update: ${path.dirname(appPath)} is not writable by this process — ` +
