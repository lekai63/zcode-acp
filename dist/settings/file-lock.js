 ...
 */
function parseLockTimestamp(value, observedAt) {
    if (typeof value !== "number")
        return null;
    if (!Number.isFinite(value) || value < 0)
        return null;
    if (value > observedAt + MAX_CLOCK_SKEW_MS)
        return null;
    return value;
}
function parseLockMetadata(raw, observedAt) {
    try {
    ...
    try {
        ...
        if (lockStat.isDirectory()) {
            ...
            if (owners.length === 1) {
                ...
                if (!(await isOwnerFileReclaimable(join(lockFile, owners[0]), ownerlessGraceMs, observedAt))) {
...
function createLockTimeoutError(filePath, waitedMs, cause) {
    const error = new Error(`Timed out after ${waitedMs}ms waiting for the ZCode file lock: ${filePath}`);
...
const processFileLockTails = new Map();
/**
...
async function acquireFileLock(filePath, options) {
    ...
    const ownerlessGraceMs = Math.min(Math.max(options.lockOwnerlessGraceMs ?? DEFAULT_OWNERLESS_GRACE_MS, 0), Math.max(Math.floor(maxWaitMs / 2), 0));
    ...
    for (let attempt = 0;; attempt += 1) {
        ...
        try {
            ...
            const currentOwners = (await readdir(lockFile)).filter((e) => e.startsWith("owner-") && e.endsWith(".json"));
            ...
            if (currentLockStat.dev !== createdLockStat.dev ||
            ...
            if (removalAttempt.removed)
                continue;
            if (removalAttempt.error !== undefined)
