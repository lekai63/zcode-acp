...
export function validateCliConfig(doc) {
    ...
    if (mcp !== undefined) {
        ...
        if (servers !== undefined && !isRecord(servers))
            return "mcp.servers must be an object";
        if (isRecord(servers)) {
            for (const [name, server] of Object.entries(servers)) {
                if (!isRecord(server))
                    return `mcp.servers.${name} must be an object`;
            }
        }
    }
    ...
    if (hooks !== undefined) {
        ...
        if (events !== undefined) {
            if (!isRecord(events))
                return "hooks.events must be an object";
            for (const [event, matchers] of Object.entries(events)) {
                ...
                if (!Array.isArray(matchers))
                    return `hooks.events.${event} must be an array`;
                for (const matcher of matchers) {
                    if (!isRecord(matcher))
                        return `hooks.events.${event} entries must be objects`;
                    if (!Array.isArray(matcher["hooks"]) || matcher["hooks"].length === 0) {
                        return `hooks.events.${event} matcher needs at least one hook`;
                    }
                    for (const hook of matcher["hooks"]) {
                        if (!isRecord(hook))
                            return "a hook entry must be an object";
 ...
 */
export function validateMcpServer(name, server) {
    if (!isRecord(server))
        return "server must be an object";
    for (const key of ["type", "url", "command"]) {
    ...
    for (const key of ["env", "headers"]) {
        ...
        if (value === undefined)
            continue;
        if (!isRecord(value))
            return `mcp.servers.${name}.${key} must be an object`;
        for (const inner of Object.values(value)) {
            if (typeof inner !== "string")
                return `mcp.servers.${name}.${key} values must be strings`;
        }
    }
...
async function saveCliConfig(mutator) {
    const { doc } = await writeJsonAtomic(zcodeCliConfigPath(), async (current) => {
 ...
 */
export async function setSkillEnabled(skillPath, enable) {
    return saveCliConfig((doc) => {
        const skills = { ...(doc.skills ?? {}) };
        if (enable)
            delete skills[skillPath];
        else
            skills[skillPath] = { enable: false };
        return applySkillsBlock(doc, skills);
    });
}
/** Drop the whole `skills` block when every entry is enabled (the app's rule). */
function applySkillsBlock(doc, skills) {
    const next = { ...doc };
 ...
 */
export async function setHooksEnabled(enabled) {
    return saveCliConfig((doc) => {
        const hooks = { ...(doc.hooks ?? {}) };
        if (enabled)
            hooks.enabled = true;
 ...
 */
export async function updateHookEntry(event, matcherIndex, hookIndex, patch) {
    if (!HOOK_EVENT_NAMES.includes(event)) {
    ...
    return saveCliConfig((doc) => {
        ...
        if (!matcher)
            throw new Error(`no matcher at index ${matcherIndex} for ${event}`);
...
export async function setPluginEnabled(pluginKey, enabled) {
...
export async function readCliConfigRaw() {
    try {
