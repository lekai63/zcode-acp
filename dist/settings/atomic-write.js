...
export async function readJsonDocument(file) {
    ...
    try {
        ...
        throw new BrokenConfigError(`${file} is not valid JSON — refusing to write (${error instanceof Error ? error.message : String(error)})`);
    }
 ...
 */
async function scanBackups(file) {
...
async function backupCurrent(file) {
    ...
    try {
        ...
        warn(`settings: backup of ${file} failed: ${error instanceof Error ? error.message : String(error)}`);
        return undefined;
    }
}
/**
 ...
 */
export async function writeJsonAtomic(file, mutator, options = {}) {
    return withFileLock(file, async () => {
        ...
        if (options.validate) {
            ...
            if (verdict !== true) {
                throw new BrokenConfigError(typeof verdict === "string"
        ...
        const temp = join(dir, `.${basename(file)}.${process.pid}.${Date.now()}.${Math.random().toString(16).slice(2)}.tmp`);
        try {
 ...
 */
export function backupStampToIso(stamp) {
