 ...
 */
export async function requireWorkflowEnabled(server) {
    ...
    if (!gate?.enabled)
        throw new WorkflowApiError(403, "workflow_disabled");
    return gate;
}
/** Workspace ref for session-less `workflows/*` calls (collectSessions shape). */
function workspaceRef(server) {
...
async function rpc(server, method, params, timeoutMs) {
    const backend = await server.ensureBackend();
    const resp = await backend.request(server.nextId(), method, params, timeoutMs);
    if (resp.error) {
        throw new WorkflowApiError(502, "backend_error", `backend ${method} failed: ${resp.error.message ?? "unknown error"}`);
    }
    return (resp.result ?? {});
}
/**
 ...
 * matching HTTP code.
 */
function throwIfOpFailed(result, method) {
    if (result["ok"] !== false)
        return;
    ...
    const code = reason === "invalid_name"
        ? 400
        : reason === "not_found"
            ? 404
...
export async function listWorkflows(server, scope) {
    await requireWorkflowEnabled(server);
    return rpc(server, "workflows/list", { workspace: workspaceRef(server), ...(scope ? { scope } : {}) }, TIMEOUT_FILE_MS);
}
/** `workflows/get` — script + meta for one saved workflow (directed scope). */
export async function getWorkflow(server, scope, name) {
    await requireWorkflowEnabled(server);
    const result = await rpc(server, "workflows/get", { workspace: workspaceRef(server), name, scope }, TIMEOUT_FILE_MS);
    throwIfOpFailed(result, "workflows/get");
    return result;
}
/** `workflows/updateMeta` — read-modify-write of the frontmatter only. */
export async function updateWorkflowMeta(server, scope, name, meta) {
    await requireWorkflowEnabled(server);
    const result = await rpc(server, "workflows/updateMeta", { workspace: workspaceRef(server), name, meta, scope }, TIMEOUT_FILE_MS);
    throwIfOpFailed(result, "workflows/updateMeta");
    return result;
}
/** `workflows/delete` — unlink by scope root (name validated before path join). */
export async function deleteWorkflow(server, scope, name) {
    await requireWorkflowEnabled(server);
    const result = await rpc(server, "workflows/delete", { workspace: workspaceRef(server), name, scope }, TIMEOUT_FILE_MS);
 ...
 */
export async function moveWorkflow(server, scope, name) {
    await requireWorkflowEnabled(server);
    void scope;
    const result = await rpc(server, "workflows/move", { workspace: workspaceRef(server), name }, TIMEOUT_FILE_MS);
 ...
 */
export async function listRuns(server, opts = {}) {
    await requireWorkflowEnabled(server);
    return rpc(server, "workflows/runs", {
 ...
 */
export function resolveWorkflowZcodeSid(server, acpSid) {
    const mapped = server.resolveSid(acpSid) ?? lookupLazySession(acpSid)?.zcodeSid;
    if (!mapped) {
        throw new WorkflowApiError(404, "unknown_session", `session ${acpSid} is not known to this bridge — cannot query its workflow runs`);
    }
    return mapped;
}
/** `v4/conversation/workflowRuns` — restart-discovery list incl. `resumable`. */
export async function conversationRuns(server, zcodeSid, limit) {
    await requireWorkflowEnabled(server);
    return rpc(server, "v4/conversation/workflowRuns", { sessionId: zcodeSid, limit: clampLimit(limit, 64, 64) }, TIMEOUT_V4_MS);
}
/** `v4/conversation/workflowRunEvents` — journal events, cursor never expires. */
export async function runEvents(server, zcodeSid, runId, afterSequence) {
    await requireWorkflowEnabled(server);
    return rpc(server, "v4/conversation/workflowRunEvents", { sessionId: zcodeSid, runId, ...(afterSequence !== undefined ? { afterSequence } : {}) }, TIMEOUT_V4_MS);
}
/** `v4/conversation/workflowRunArtifacts` — artifact inventory with versions. */
export async function runArtifacts(server, zcodeSid, runId) {
    await requireWorkflowEnabled(server);
    return rpc(server, "v4/conversation/workflowRunArtifacts", { sessionId: zcodeSid, runId }, TIMEOUT_V4_MS);
}
/** `v4/conversation/workflowRunArtifactData` — board item pagination. */
export async function runArtifactData(server, zcodeSid, runId, artifactId, afterSequence, limit) {
    await requireWorkflowEnabled(server);
    return rpc(server, "v4/conversation/workflowRunArtifactData", {
        sessionId: zcodeSid,
        runId,
        artifactId,
...
export async function runArtifactRead(server, zcodeSid, runId, artifactId, version, offset, limit) {
    await requireWorkflowEnabled(server);
    return rpc(server, "v4/conversation/workflowRunArtifactRead", {
        sessionId: zcodeSid,
        runId,
        artifactId,
        version,
...
export async function runWorkspace(server, zcodeSid, runId) {
    await requireWorkflowEnabled(server);
    return rpc(server, "v4/conversation/workflowRunWorkspace", { sessionId: zcodeSid, runId }, TIMEOUT_V4_MS);
}
/** `v4/conversation/workflowRunNodeResult` — one node's bounded result. */
export async function runNodeResult(server, zcodeSid, runId, siteId, ordinal) {
    await requireWorkflowEnabled(server);
    return rpc(server, "v4/conversation/workflowRunNodeResult", { sessionId: zcodeSid, runId, siteId, ordinal }, TIMEOUT_V4_MS);
}
 ...
 */
function codeForCommandReason(reasonCode) {
    if (reasonCode.includes("session_busy") || reasonCode.includes("not_resumable"))
        return 409;
    if (reasonCode.includes("not_found"))
        return 404;
    if (reasonCode.includes("invalid_name") || reasonCode.includes("invalid_args"))
        return 400;
    if (reasonCode.includes("compile_failed"))
        return 422;
    if (reasonCode.includes("capabilityUnsupported"))
        return 501;
    return 502;
}
/** The short token for an ack rejection: the fault namespace's last segment. */
function reasonToken(reasonCode, status) {
    if (reasonCode) {
 ...
 */
async function sendCommand(server, type, zcodeSid, payload) {
    const backend = await server.ensureBackend();
    const envelope = {
    ...
    if (resp.error) {
        throw new WorkflowApiError(502, "command_failed", `v4/command ${type} failed: ${resp.error.message ?? "unknown error"}`);
    }
    return (resp.result ?? {});
}
/** Throw the ack-shaped rejection (status ≠ accepted). */
function throwAckRejection(ack, type) {
    const reasonCode = typeof ack.reasonCode === "string" ? ack.reasonCode : "";
    const fallback = `v4/command ${type} ${typeof ack.status === "string" ? ack.status : "failed"}` +
        (reasonCode ? ` (${reasonCode})` : "");
    throw new WorkflowApiError(codeForCommandReason(reasonCode), reasonToken(reasonCode, ack.status), typeof ack.message === "string" && ack.message ? ack.message : fallback);
}
/**
 ...
 * pre-check.
 */
async function mapProvidedSession(server, acpSid) {
    try {
        return await ensureRealSession(server, acpSid);
    }
    catch (e) {
        throw new WorkflowApiError(404, "unknown_session", `session ${acpSid} could not be resolved: ${messageOf(e)}`);
    }
}
/**
 ...
 */
async function discardCreatedSession(server, acpSid, zcodeSid) {
    ...
    if (zcodeSid)
        server.acpSidByZcodeSid.delete(zcodeSid);
    server.backendLoadedSessions.delete(acpSid);
    if (!zcodeSid)
        return;
    try {
        const backend = await server.ensureBackend();
        const resp = await backend.request(server.nextId(), "session/close", { sessionId: zcodeSid }, 10_000);
        if (resp.error) {
 ...
 */
export async function startSavedWorkflow(server, input) {
    await requireWorkflowEnabled(server);
    let zcodeSid;
    let createdAcpSid = null;
    ...
    try {
        ...
        if (createdAcpSid)
            await discardCreatedSession(server, createdAcpSid, zcodeSid);
 ...
 */
export async function resumeWorkflowRun(server, input) {
    await requireWorkflowEnabled(server);
    ...
    if (ack.status !== "accepted")
        throwAckRejection(ack, "resumeWorkflowRun");
}
 ...
 */
const WORKFLOW_CREATE_PROMPT_PROJECT = "Help me design a workflow and save it to this project once it works: ";
const WORKFLOW_CREATE_PROMPT_GLOBAL = 'Help me design a workflow and save it as a global workflow (scope: "global") with SaveWorkflow once it works: ';
/** The prefilled prompt for the "create via conversation" entry point. */
export function workflowCreatePrompt(scope) {
