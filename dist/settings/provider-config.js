...
function normalize(doc) {
    ...
    if (Array.isArray(config.providerOrder))
        out.config.providerOrder = config.providerOrder;
 ...
 */
export function encodeProviderConfig(doc) {
    return JSON.stringify(doc, null, 2);
}
/** Structural self-check run before the bytes land. */
export function validateProviderConfig(doc) {
    const file = doc;
    ...
    if (!Array.isArray(rules))
        return "config.providerConfigRules.providerRules must be an array";
    const seen = new Set();
    for (const rule of rules) {
        ...
        if (seen.has(rule.providerId))
            return `duplicate providerId ${rule.providerId}`;
        ...
        const accessType = rule.config?.access?.type;
    ...
    for (const list of [modelRules.providerModelRules, modelRules.manualProviderModelRules]) {
        if (!Array.isArray(list))
            return "model rule lists must be arrays";
...
function assertNotAccountProvider(providerId) {
    if (providerId.startsWith(ACCOUNT_PROVIDER_PREFIX)) {
        throw new Error(`provider ${providerId} is account-managed — its models come from the coding plan, not this file`);
    }
}
/**
 ...
 */
async function saveProviderConfig(mutator) {
    const file = zcodePersonalProviderPath();
    const { doc } = await writeJsonAtomic(file, (current) => {
...
export async function updateProvider(providerId, patch) {
    ...
    return saveProviderConfig((current) => {
        const rule = current.config.providerConfigRules.providerRules.find((r) => r.providerId === providerId);
        if (!rule) {
            throw new Error(`provider ${providerId} has no rule in provider_config.json — only providers declared here can be edited`);
        }
        if (patch.enabled !== undefined)
            rule.enabled = patch.enabled;
        if (patch.providerName !== undefined)
            rule.providerName = patch.providerName;
    });
}
/**
 ...
 */
export async function upsertModel(providerId, modelId, patch) {
    assertNotAccountProvider(providerId);
    ...
    return saveProviderConfig((current) => {
        ...
        const existing = rules.providerModelRules.find((r) => r.providerId === providerId && r.modelId === modelId) ??
            rules.manualProviderModelRules.find((r) => r.providerId === providerId && r.modelId === modelId);
        if (existing) {
        ...
        const provider = current.config.providerConfigRules.providerRules.find((r) => r.providerId === providerId);
        const list = provider?.config?.personalModelIds ?? [];
        if (!list.includes(modelId)) {
            ...
            if (provider) {
                ...
                const order = provider.config.modelOrder ?? [];
                if (!order.includes(modelId))
                    provider.config.modelOrder = [...order, modelId];
            }
        }
    });
}
...
export async function removeModel(providerId, modelId) {
    ...
    const result = await saveProviderConfig((current) => {
        ...
        rules.providerModelRules = rules.providerModelRules.filter((r) => !(r.providerId === providerId && r.modelId === modelId));
        rules.manualProviderModelRules = rules.manualProviderModelRules.filter((r) => !(r.providerId === providerId && r.modelId === modelId));
        ...
        if (provider?.config) {
            for (const key of ["personalModelIds", "modelOrder"]) {
                const list = provider.config[key];
                if (Array.isArray(list)) {
    ...
    if (missing)
        throw new Error(`model ${modelId} has no rule for provider ${providerId}`);
    return result;
}
/**
 ...
 */
function mergeModelConfig(base, patch) {
    const out = { ...base };
...
export async function readProviderConfigRaw() {
    try {
