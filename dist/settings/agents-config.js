...
function validateAgentsState(doc) {
...
async function saveAgentsState(mutator) {
    const { doc } = await writeJsonAtomic(zcodeAgentsStatePath(), (current) => mutator(current), { validate: (d) => validateAgentsState(d) });
    return doc;
}
/**
 ...
 */
export async function setAgentEnabled(name, enabled) {
 ...
 */
export async function setBuiltInAgentModel(name, selection) {
    if (!BUILT_IN_AGENTS.includes(name)) {
    ...
    return saveAgentsState((doc) => {
        ...
        if (Object.keys(current).length === 0)
            delete next.builtInModelSelectionOverrides;
        else
            next.builtInModelSelectionOverrides = current;
        return next;
    });
}
// ---------- agent markdown ----------
...
function unquote(value) {
    ...
    if (!value.startsWith('"'))
        return out; // escapes only apply in double quotes
 ...
 */
export function serializeAgentMarkdown(frontmatter, body) {
    const lines = [
    ...
    if (typeof frontmatter.maxTurns === "number")
        lines.push(`maxTurns: ${frontmatter.maxTurns}`);
 ...
 */
async function writeAgentFile(filePath, content) {
...
async function listAgentFiles() {
    ...
    for (const entry of entries) {
        if (!entry.endsWith(".md") && !entry.endsWith(".markdown"))
            continue;
        try {
            if ((await stat(path.join(dir, entry))).isFile())
                out.push(entry);
        }
        catch {
            /* vanished mid-scan */
...
export async function listAgents() {
    ...
    const out = BUILT_IN_AGENTS.map((name) => ({
        name,
        fileName: "",
        path: "",
    ...
    for (const fileName of await listAgentFiles()) {
        ...
        if (!split)
            continue; // no frontmatter — the runtime ignores it too
...
function resolveAgentFile(name) {
    if (!isValidAgentName(name)) {
        throw new Error(`invalid agent name '${name}' — 3-50 characters of letters, digits and hyphens`);
    }
    const dir = zcodeAgentsDir();
...
export async function createAgent(input) {
    ...
    if (input.color !== undefined && !AGENT_COLORS.includes(input.color)) {
    ...
    if (input.thoughtLevel !== undefined)
        frontmatter.thoughtLevel = input.thoughtLevel;
 ...
 */
export async function updateAgent(name, patch) {
    ...
    if (!reparsed)
        throw new Error("serialized agent has no frontmatter");
    ...
    for (const [key, before] of Object.entries(next)) {
        ...
        if (typeof before === "object")
            continue; // lists compared by their own path
