...
const BUSY_RETRY_DELAYS_MS = [200, 400];
/**
...
function openReadOnly(dbPath) {
    ...
    for (let attempt = 0;; attempt += 1) {
        try {
            ...
            if (delay === undefined || !isBusyError(error))
                return null;
            Atomics.wait(sleeper, 0, 0, delay);
        }
    }
}
const EMPTY_SNAPSHOT = (range) => ({
    available: false,
    range,
    summary: {
        totalTokens: 0,
 ...
 */
export async function readUsageStats(range = "7d") {
    ...
    try {
        ...
        const dayRows = db
            .prepare(`SELECT date(started_at / 1000, 'unixepoch', 'localtime') AS day,
                model_id,
                COALESCE(SUM(computed_total_tokens), 0) AS total_tokens
           FROM model_usage
          WHERE started_at > ?
        ...
        const scalar = (sql) => {
            try {
                const row = db.prepare(sql).get(floor);
        ...
        const sessionCount = scalar("SELECT COUNT(DISTINCT session_id) AS c FROM turn_usage WHERE started_at > ?");
        ...
        warn(`settings: usage aggregation failed: ${error instanceof Error ? error.message : String(error)}`);
        return EMPTY_SNAPSHOT(range);
    }
    finally {
        try {
