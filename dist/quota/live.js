...
import { composeQuotaDock, formatGoDockSegment, formatOcDockSegment, formatQuotaDock, } from "./format.js";
 ...
 */
export function startQuotaRefresher(server) {
    if (state || !server.marttyClientSeen)
        return;
    const entry = {
        server,
...
export function forceRefreshQuota() {
    if (!state)
        return Promise.resolve();
    return refresh(state);
}
/** Stop and forget the singleton (test helper). */
export function resetQuotaRefresherForTest() {
    if (state)
...
async function runRefresh(server) {
...
export async function fetchDockText(env = process.env) {
        ...
        try {
            ...
            if (resp.ok) {
                ...
                if (body.formatted === null)
                    return null;
                if (typeof body.formatted === "string")
 ...
 * emit did land.
 */
export function scheduleQuotaDockBackstop(server) {
    if (server.quotaDock === null)
        return;
    setImmediate(() => {
        if (server.quotaDock !== null)
            void emitQuotaOptions(server).catch(() => { });
    });
}
/**
 ...
 */
async function emitQuotaOptions(server) {
    if (server.marttyConnectionRoots.size === 0)
        return;
    ...
    const byZcodeSid = new Map();
    const emitFor = async (acpSid) => {
        ...
        await enqueueSessionSend(acpSid, async () => {
            const results = await Promise.allSettled(targets.map((cx) => cx.notify("session/update", {
                sessionId: acpSid,
                update: { sessionUpdate: "config_option_update", configOptions: options },
            })));
            for (const r of results) {
                if (r.status === "rejected") {
                    warn(`quota-dock: config_option_update failed on one client: ` +
    ...
    for (const acpSid of acpSids)
        await emitFor(acpSid);
}
/** clientConnectionRoot equivalent for a raw ClientLike (see utils.ts). */
function connectionRootOf(cx) {
    return cx.connectionContext;
}
