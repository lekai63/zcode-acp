 ...
 */
async function lookupMonthlyReset(apiKey) {
    try {
        ...
        if (period !== undefined) {
            const raw = period.Time ?? period.time;
            const valid = period.Valid ??
                period.valid;
        ...
        if (typeof createdRaw === "string") {
            ...
            if (Number.isFinite(created))
                return nextMonthlyAnniversary(created, Date.now());
        }
        return undefined;
    }
    catch {
 ...
 */
export function nextMonthlyAnniversary(createdMs, now) {
    ...
    for (let year = nowDate.getUTCFullYear(); year <= nowDate.getUTCFullYear() + 1; year++) {
        for (let month = year === nowDate.getUTCFullYear() ? nowDate.getUTCMonth() : 0; month < 12; month++) {
 ...
 */
export async function queryOcUsage() {
    ...
    try {
        ...
        if (resp.status === 401 || resp.status === 403) {
            ...
            const window = (key) => validFraction(limits[key]?.usage) ? limits[key].usage : undefined;
            ...
            if (session !== undefined || weekly !== undefined || monthly !== undefined) {
                ...
                if (monthly !== undefined) {
                    ...
                    if (monthlyResetAt !== undefined)
                        result.monthlyResetAt = monthlyResetAt;
                }
            }
            else {
