 ...
 */
export function workflowEventLine(ev) {
    const p = ev.payload ?? {};
    const str = (k) => typeof p[k] === "string" ? p[k] : undefined;
    const num = (k) => typeof p[k] === "number" ? p[k] : undefined;
    switch (ev.type) {
        case "actor-created": {
        ...
        default: {
            ...
            if (outcome !== "ok" && outcome !== "failed" && outcome !== "cancelled")
                return null;
            const ordinal = num("ordinal");
 ...
 */
async function emitCardUpdate(run, notice) {
...
function teardown(run) {
    ...
    run.timer = null;
    run.stopped = true;
    if (activeRuns.get(run.runId) === run)
...
async function pollTick(run) {
    ...
    try {
        ...
        for (let page = 0; page < MAX_PAGES_PER_TICK; page++) {
            const resp = await backend.request(run.server.nextId(), "v4/conversation/workflowRunEvents", {
...
async function handleError(run, error) {
    if (error.code === -32601) {
        ...
        log(`workflow poller: backend has no v4/conversation/workflowRunEvents — ` +
    ...
    if (run.consecutiveErrors >= MAX_CONSECUTIVE_ERRORS) {
        log(`workflow poller: ${run.consecutiveErrors} consecutive failures — ` +
            `stopping run ${run.runId.slice(-8)}`);
 ...
 */
export function armWorkflowRunPoller(server, zcodeSid, opts) {
...
export function stopWorkflowRunPoller(runId) {
...
export function stopAllWorkflowRunPollers() {
 ...
 */
export function stopPollersForSession(zcodeSid) {
    for (const run of [...activeRuns.values()]) {
        if (run.zcodeSid === zcodeSid)
            teardown(run);
    }
}
/** Test/observability hook: is a poller currently armed for this runId? */
export function workflowPollerActive(runId) {
