...
function diaryFile(now = new Date()) {
 ...
 */
export function appendDiary(line) {
    if (!diaryEnabled())
        return;
    try {
...
export function isStdoutPipeBreak(reason) {
    const err = reason;
    const code = err?.code;
    if (code === "EPIPE")
        return true;
...
export function resetCrashGuardsForTest() {
    installed = false;
}
//# sourceMappingURL=crash-guards.js.map