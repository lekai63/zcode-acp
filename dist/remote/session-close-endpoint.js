 ...
 */
export function serveTerminateDecision(advertisedCount, env = process.env) {
    if ((env.ZCODE_ACP_REMOTE_ORIGIN ?? "").trim() !== "serve")
        return null;
 ...
 */
function advertisedSessionCount(server) {
    let count = server.remoteCreatedSessions.size;
    for (const [sid, summary] of server.sessionSummaries) {
        if (summary.hasActivity && !server.remoteCreatedSessions.has(sid))
            count++;
    }
    return count;
}
 ...
 */
export function tuiTeardownPids(tuiPgid, ppid) {
    const out = [];
    for (const pid of [tuiPgid, ppid]) {
        if (Number.isInteger(pid) && pid > 1 && !out.includes(pid))
            out.push(pid);
    }
    return out;
}
 ...
 * signal forms miss.
 */
function terminateAfterFlush(decision, res) {
    const run = () => {
        if (decision.tuiPgid !== undefined) {
            try {
                ...
                log(`remote: TUI group signal failed ` +
...
async function handleClose(server, req, res, sessionId) {
    ...
    if (zcodeSid)
        stopPollersForSession(zcodeSid);
    ...
    if (zcodeSid && server.backend && !server.backend.isDead) {
        try {
            ...
            log(`remote: session/close send failed (ignored): ` +
    ...
    if (terminate !== null) {
        ...
        await Promise.all(server
            ...
            .map((sid) => server.clients.notifyEach("$/zcode/session_closed", () => ({ sessionId: sid }))));
    }
    sendJson(res, 200, { ok: true });
    if (terminate?.terminate)
        terminateAfterFlush(terminate, res);
}
/**
 ...
 */
export function createSessionCloseHandler(server) {
...
//# sourceMappingURL=session-close-endpoint.js.map