...
import { chmodSync, mkdirSync, mkdtempSync, readdirSync, realpathSync, statSync, unlinkSync, writeFileSync, } from "node:fs";
 ...
 */
export function resolveTerminalLaunch(env, prefs = remoteTerminalPrefs(env)) {
 ...
 */
export function resolveTerminalLaunches(prefs) {
    if (prefs.enabled === false)
        return [];
    if (prefs.command)
        return [{ kind: "shell", command: prefs.command }];
 ...
 */
export function ghosttyTabAppleScript(app, scriptPath) {
    return [
...
export function writeTuiScript(workspace, cliJs, env) {
    ...
    try {
        ...
        for (const entry of readdirSync(dir)) {
            if (!entry.startsWith("tui-") || !entry.endsWith(".command"))
                continue;
            const stale = path.join(dir, entry);
            try {
                if (statSync(stale).mtimeMs < cutoff)
                    unlinkSync(stale);
            }
            catch {
        ...
        warn(`hub: cannot place the TUI script in ${dir} (${e instanceof Error ? e.message : String(e)}) — using the system tmpdir`);
    }
...
async function spawnTerminalTui(opts) {
    ...
    // app cold-starts.
    const openTimeoutMs = launch.kind === "ghosttyScript" ? TERMINAL_SCRIPT_TIMEOUT_MS : TERMINAL_OPEN_TIMEOUT_MS;
    const errChunks = [];
...
async function defaultSpawnServe(opts) {
    ...
    child.stderr?.on("data", (d) => {
        for (const line of d.split("\n")) {
            if (line.trim())
                warn(`serve-bridge[${opts.cwd}]: ${line}`);
        }
    });
...
export function resetQuotaCacheForTest() {
    quotaCache = null;
    quotaInflight = null;
}
/**
...
export function resetDockCacheForTest() {
    dockCache = null;
}
/**
 ...
 */
export function sanitizeTabTitle(v) {
    if (typeof v !== "string")
        return undefined;
    const clean = v
        .replace(/[\p{Cc}]/gu, " ")
 ...
 */
function fetchBridgeSessions(port, query = "") {
...
async function newestJsMtime(dir) {
    let newest = null;
 ...
 */
async function diskCodeIsNewer(paths, startedAt, runningFingerprint) {
...
export function startHub(options) {
     ...
     */
    const hubFingerprint = options.hubFingerprint !== undefined ? options.hubFingerprint : readCodeFingerprint();
    ...
    const findServeInstances = (workspacePath) => {
        ...
        return Array.from(instances.values()).filter((e) => e.origin === "serve" && canonicalPath(e.workspace) === wanted);
    };
    /** The live serve instance for a workspace, if any (per-workspace dedupe). */
    const findServeInstance = (workspacePath) => findServeInstances(workspacePath)[0];
    /**
     ...
     */
    const incubateServe = async (workspacePath, kind, sessionId, tabTitle) => {
            ...
            while (li < launches.length) {
                ...
                try {
                    ...
                    if (opened) {
                        ...
                        spawnError = null;
                        log(`hub: spawned terminal TUI via ${attempt.kind === "shell"
                            ? "command template"
                            : attempt.kind === "ghosttyScript"
                                ? `Ghostty (${attempt.app})`
                                : attempt.kind === "warpUri"
                                    ? `Warp (${attempt.app})`
                                    : `open -a ${attempt.app}`} for ${workspacePath} (pid ${opened.pid})`);
                        return opened;
                    }
                    warn(`hub: terminal launch ${li}/${launches.length} failed to open — trying the next preference`);
                }
                catch (e) {
        ...
        try {
            ...
            if (opened) {
                ...
                if (!spawned)
                    throw new Error("headless spawn returned null");
                child = spawned;
                child.once("error", (e) => {
                    spawnError = e;
                });
        ...
        log(`hub: incubating ${kind === "resume" ? "resume TUI" : kind === "tui" ? "terminal TUI" : "serve bridge"} for ${workspacePath} (pid ${child.pid})`);
        ...
        const preexisting = new Set(kind === "serve" ? [] : findServeInstances(workspacePath).map((e) => e.id));
        for (;;) {
            await new Promise((resolve) => setTimeout(resolve, SERVE_REGISTER_POLL_MS).unref?.());
            ...
            if (Date.now() > deadline) {
                ...
                if (kind === "tui") {
                    ...
                    if (fallback) {
                        warn(`hub: TUI for ${workspacePath} slow to register — ` +
                ...
                if (kind !== "serve" && li < launches.length) {
                    warn(`hub: terminal TUI for ${workspacePath} never registered — trying the next terminal preference`);
                ...
                if (kind !== "serve" && !rescued) {
                    rescued = true;
                    warn(`hub: ${kind === "resume" ? "resume TUI" : "terminal TUI"} for ${workspacePath} ` +
                    ...
                    try {
                        ...
                        if (!rescuedChild)
                            throw new Error("rescue spawn returned null");
                        child = rescuedChild;
                        child.once("error", (e) => {
                            spawnError = e;
                        });
                        ...
                        warn(`hub: headless rescue spawn failed: ${e instanceof Error ? e.message : String(e)}`);
     ...
     * their own window.
     */
    const joinIncubation = (workspacePath, kind, sessionId, tabTitle) => {
        const mapKey = kind === "resume" ? `${workspacePath}\u0000resume\u0000${sessionId ?? ""}` : workspacePath;
        ...
        promise
            ...
            if (current && current.promise === promise)
                serveIncubations.delete(mapKey);
        });
        return promise;
    };
    /**
    ...
    server.on("upgrade", (req, socket, head) => {
        ...
        bridge.once("open", () => {
            if (clientGone)
                return; // terminated above; "open" can no longer fire
    ...
    async function handleHttp(req, res) {
        ...
        if (url.pathname === "/api/instances" && req.method === "POST") {
            ...
            const rawPath = body?.workspacePath;
            ...
            if (resumeSid) {
                ...
                if (titleSource) {
                    const row = await withTimeout(fetchBridgeSessions(titleSource.port, "?limit=200")
                        .then((payload) => (payload.sessions ?? []).find((s) => s.sessionId === resumeSid))
                        .catch(() => undefined), TITLE_LOOKUP_BUDGET_MS);
                    tabTitle = sanitizeTabTitle(row?.title);
                }
        ...
        if (url.pathname === "/api/upgrade" && req.method === "POST") {
            ...
            if (check.newer) {
                restartSoon(`hub: on-disk code is newer (${check.reason}: ${check.diskVersion ?? "rebuilt dist"}) — restarting onto it`);
            }
            return;
        }
        ...
        if (fsMatch && (req.method === "GET" || req.method === "HEAD")) {
            ...
            const upstream = httpGet({ host: "127.0.0.1", port: entry.port, path: fsMatch[2] + url.search }, (up) => {
        ...
        const sessionOpMatch = url.pathname.match(/^\/api\/instances\/([^/]+)\/sessions\/([^/]+)\/(close|rename)$/);
        ...
        if ((url.pathname === "/api/register" || url.pathname === "/api/unregister") &&
            ...
            const newerVersion = typeof body.version === "string" && compareVersions(body.version, AGENT_INFO.version) > 0;
            ...
            if (stale) {
                restartSoon(`hub: bridge ${body.version} is newer than hub ${AGENT_INFO.version} — restarting to upgrade`);
            }
    ...
    const pongResponsive = new WeakSet();
