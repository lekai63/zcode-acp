...
export function sandboxBorn(env = process.env) {
...
export function buildHubRelaunchPlist(opts) {
    ...
    const pair = ([k, v]) => `        <key>${xmlEscape(k)}</key>\n        <string>${xmlEscape(String(v))}</string>`;
...
export function selfRelaunchOutsideSandbox(opts) {
    ...
    if (!loaded) {
        warn(`hub: launchd bootstrap failed (${lastError}) — running sandboxed; ` +
    ...
    try {
        ...
        warn(`hub: launchd kickstart failed (${e instanceof Error ? e.message : String(e)}) — ` +
...
//# sourceMappingURL=hub-sandbox.js.map