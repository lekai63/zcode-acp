...
const PINNED_PREFIXES = ["ZCODE_ACP_", "DSH_TUI_", "MARTTY_"];
 ...
 */
function unionPath(shellPath, callerPath) {
    const seen = new Set();
    const out = [];
 ...
 */
export async function envWithLoginShell(env = process.env) {
 ...
 */
function probeCommand(shell) {
    const name = shell.split("/").pop() ?? "";
    const rcFiles = name === "zsh"
        ? ['"$HOME/.zshrc"']
        : name === "bash"
...
async function probe() {
    try {
        ...
        for (const rec of stdout.toString("utf8").split("\0")) {
            ...
            if (!ENV_NAME_RE.test(key) || NEVER_COPY.has(key))
                continue;
            out[key] = value;
            added += 1;
        }
        ...
        failedAt = Date.now();
        warn(`env: login-shell probe failed (${e instanceof Error ? e.message : String(e)}) — ` +
            `falling back to the inherited environment`);
        return null;
    }
}
/** Test seam: drop the caches so the next call re-probes. */
export function resetLoginShellEnvForTest() {
    cached = null;
    inflight = null;
    failedAt = 0;
}
//# sourceMappingURL=login-shell-env.js.map