 ...
 */
export async function collectSessions(server) {
    ...
    for (const [acpSid, summary] of server.sessionSummaries) {
        ...
        if (!zcodeSid)
            continue; // pure placeholder — no backend session behind it
    ...
    for (const acpSid of server.remoteCreatedSessions) {
        if (server.resolveSid(acpSid))
            continue; // materialized — covered above
        if (advertised.has(acpSid))
            continue;
    ...
    if (backend && !backend.isDead && live.size > 0) {
        try {
            ...
            const resp = await backend.request(server.nextId(), "session/list", { workspace: { workspacePath: cwd, workspaceKey: cwd } }, 10_000);
            if (!resp.error) {
                const sessions = (resp.result ?? {}).sessions ?? [];
                for (const s of sessions) {
                    if (!s.sessionId)
                        continue;
                    const cur = live.get(s.sessionId);
                    if (!cur)
                        continue; // not running here — the store entry is history
...
export async function startRemoteEndpoint(server, app, config) {
    ...
    const httpServer = createServer((req, res) => {
        ...
        if (path === "/acp")
            ...
            sessionCloseHandler(req, res, closeMatch[1]);
        else if (renameMatch)
            sessionRenameHandler(req, res, renameMatch[1]);
        else if (path === "/sessions")
            sessionListHandler(req, res);
        else {
    ...
    const registerOnce = async () => {
        ...
        try {
            const res = await postJson(`http://127.0.0.1:${config.hubPort}/api/register`, payload(lastSessions));
            if (res.status === 401) {
                ...
                if (!authRejected) {
                    authRejected = true;
                    warn("remote: hub rejected the token (401) — registration keeps retrying; " +
