...
export function parseRemoteConfig(env = process.env) {
    if (!remoteEnabledLive(env))
        return null;
    const { token, hubPort, hubHost, bridgePort } = mergeCommon(env);
    if (!token) {
        warn("remote: enabled but no token (config file or ZCODE_ACP_REMOTE_TOKEN) — " +
 ...
 */
export function remoteEnabledLive(env = process.env) {
    const file = loadUserConfig(env).remote ?? {};
 ...
 */
export function remoteTerminalPrefs(env = process.env) {
