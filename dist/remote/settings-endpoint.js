...
import { removeModel, readProviderConfig, updateProvider, upsertModel, } from "../settings/provider-config.js";
import { readCliConfig, readHooks, removeMcpServer, setHooksEnabled, setMcpServerEnabled, updateHookEntry, upsertMcpServer, validateMcpServer, } from "../settings/cli-config.js";
import { BUILT_IN_AGENTS, createAgent, deleteAgent, listAgents, setAgentEnabled, setBuiltInAgentModel, updateAgent, } from "../settings/agents-config.js";
import { copySkillToUser, deleteSkill, listSkills, setSkillEnabledByPath, } from "../settings/skills.js";
...
import { appBundlePath, appUpdateState, checkForAppUpdate, fetchReleaseManifest, installedAppVersion, isNewerVersion, resetAppUpdateStateForTest as resetAppUpdateForTest, startAppUpdate, } from "../settings/app-update.js";
import { readUsageStats } from "../settings/usage-stats.js";
import { conversationRuns, deleteWorkflow, getWorkflow, listRuns, listWorkflows, moveWorkflow, requireWorkflowEnabled, resolveWorkflowZcodeSid, resumeWorkflowRun, runArtifactData, runArtifactRead, runArtifacts, runEvents, runNodeResult, runWorkspace, startSavedWorkflow, updateWorkflowMeta, workflowCreatePrompt, WorkflowApiError, } from "../settings/workflow.js";
import { codingPlanProviderIds, isCodingPlanProvider, markResetHistoryRead, readResetStatus, requestResetOpportunity, resolveResetAuthorization, useResetCard, } from "../settings/coding-plan.js";
...
function statusFor(error) {
    if (error instanceof BadRequest)
        return 400;
    if (error instanceof BrokenConfigError)
        return 500;
    ...
    if (error instanceof Error) {
        ...
        if (codes.some((c) => /^(ECONNRESET|ECONNREFUSED|ENOTFOUND|EAI_AGAIN|ECONNABORTED|EPIPE)$/u.test(c) ||
            // undici's own failure family — e.g. a destroyed socket surfaces as
            // cause.code UND_ERR_SOCKET ("fetch failed" with no errno).
            /^UND_ERR_/u.test(c))) {
            return 502;
        }
        if (codes.some((c) => /^(EACCES|EPERM|EISDIR|ENOTDIR)$/u.test(c)))
            return 500;
    }
    return 400;
}
 ...
 */
export function createSettingsHandler(server) {
    return (req, res) => {
        ...
        void route(req, res, url, path, server ?? null).catch((error) => {
            ...
            warn(`settings: unhandled route error: ${error instanceof Error ? error.message : String(error)}`);
            if (!res.writableEnded)
                sendError(res, 500, "internal error");
        });
    };
}
async function route(req, res, url, path, server) {
    const method = req.method ?? "GET";
    // ---- reads ----
    if (method === "GET") {
        if (path === "/settings/all")
            return void (await handleAll(res, server));
        if (path === "/settings/models")
            return void (await handleModels(res));
        if (path === "/settings/skills")
            return void (await handleSkills(res));
        if (path === "/settings/mcp")
            return void (await handleMcp(res));
        if (path === "/settings/hooks")
            return void (await handleHooks(res));
        if (path === "/settings/agents")
            return void (await handleAgents(res));
        if (path === "/settings/backups")
            return void (await handleBackups(res));
        if (path === "/settings/usage")
            return void (await handleUsage(res, url));
        if (path === "/settings/quota")
            return void (await handleQuota(res));
        if (path === "/settings/reset-cards")
            return void (await handleResetCards(res, url));
        if (path === "/settings/pending-restart")
            return void (await handlePendingRestart(res, server));
        if (path === "/settings/app-update")
            return void (await handleAppUpdate(res, url));
        // ---- dynamic-workflow management (per-instance; see the section comment
        // below the write block) ----
        if (path === "/settings/workflows")
            return void (await handleWorkflowList(res, server, url));
    ...
    if (method === "PUT" || method === "POST" || method === "DELETE") {
        ...
        if (method === "DELETE" && path.startsWith("/settings/models/")) {
            ...
            if (sep <= 0)
                return sendError(res, 400, "expected /settings/models/{providerId}/{modelId}");
        ...
        if (method === "DELETE" && path.startsWith("/settings/mcp/")) {
            ...
            return void (await guard(res, async () => {
                ...
                return { ok: true, effect: "needs-restart" };
            }));
        }
...
async function guard(res, fn) {
    try {
        const body = await fn();
        if (body.effect === "needs-restart")
            pendingRestartWrites += 1;
...
export function resetPendingRestartForTest() {
    pendingRestartWrites = 0;
}
async function handlePendingRestart(res, server) {
    ...
    if (!server) {
        sendError(res, 409, "pending-restart is per-bridge — use /api/instances/{id}/settings/pending-restart");
        return;
    }
...
// ---------- reads ----------
async function handleAll(res, server) {
    ...
    const workflow = server ? await readWorkflowGateBlock(server) : { available: false };
 ...
 */
async function readWorkflowGateBlock(server) {
    try {
        if (!server.backendWorkflowGate) {
            await server.ensureBackend().catch(() => undefined);
        }
 ...
 */
async function resetCardEligibility() {
...
async function handleModelsPayload() {
...
async function handleModels(res) {
    await guard(res, async () => ({ ok: true, models: await handleModelsPayload() }));
}
async function handleSkills(res) {
    await guard(res, async () => ({ ok: true, skills: await listSkills() }));
}
async function readMcpView() {
...
async function handleMcp(res) {
    await guard(res, async () => ({ ok: true, mcp: await readMcpView() }));
}
async function handleHooks(res) {
    await guard(res, async () => {
...
async function handleBackups(res) {
    await guard(res, async () => {
        const [provider, cli] = await Promise.all([
            listBackups(zcodePersonalProviderPath()),
            listBackups(zcodeCliConfigPath()),
        ]);
        return { ok: true, backups: { providerConfig: provider, cliConfig: cli } };
    });
}
// ---------- writes ----------
async function handleUpdateProvider(res, providerId, body) {
    const enabled = bool(body, "enabled");
...
async function handleUpsertModel(res, body) {
    ...
    await guard(res, async () => {
        await upsertModel(providerId, modelId, {
            ...
            ...(Array.isArray(body["reasoningLevels"])
                ? { optionSpecs: { reasoningLevel: { values: body["reasoningLevels"] } } }
                : {}),
        });
        return { ok: true, effect: "immediate" };
    });
}
async function handleRemoveModel(res, providerId, modelId) {
    await guard(res, async () => {
        await removeModel(providerId, modelId);
        return { ok: true, effect: "immediate" };
    });
}
async function handleSkillEnable(res, body) {
...
async function handleSkillCopy(res, body) {
    const skillPath = str(body, "path");
    if (!skillPath) {
    ...
    await guard(res, async () => {
        ...
        return { ok: true, effect: "immediate", path: target };
    });
}
async function handleSkillDelete(res, skillPath) {
    await guard(res, async () => {
        await deleteSkill(skillPath);
        return { ok: true, effect: "immediate" };
    });
}
async function handleMcpUpsert(res, name, body) {
    ...
    await guard(res, async () => {
        await upsertMcpServer(name, body);
        return { ok: true, effect: "needs-restart" };
    });
}
async function handleMcpEnable(res, body) {
...
async function handleHookUpdate(res, rest, body) {
    const parts = rest.split("/");
    if (parts.length !== 2) {
        sendError(res, 400, "expected /settings/hooks/{event}/{matcherIndex}");
        return;
    }
    const event = segment(parts[0]);
...
async function handleAgentUpsert(res, name, body) {
    await guard(res, async () => {
        ...
        if (BUILT_IN_AGENTS.includes(name)) {
            ...
            if (!clearing && (providerId === undefined || modelId === undefined)) {
                throw new Error("a built-in agent override needs providerId and modelId; send nulls to clear");
            }
            if (clearing) {
        ...
        const patch = {
            ...
            ...("thoughtLevel" in body
                ? { thoughtLevel: body["thoughtLevel"] ?? null }
                : {}),
        };
        let agent;
        try {
            ...
            if (description === undefined)
                throw new Error("description is required to create an agent");
        ...
        return { ok: true, effect: "needs-restart", agent };
    });
}
async function handleAgentEnable(res, name, body) {
    const enable = bool(body, "enable");
    if (enable === undefined) {
...
async function handleBackupRestore(res, body) {
    ...
    await guard(res, async () => {
        ...
        if (doc === null)
            throw new Error("backup is not a readable JSON object");
        ...
        const effect = which === "providerConfig" ? "immediate" : "needs-restart";
        return { ok: true, effect, restored: backupPath };
    });
}
// ---------- usage / quota / reset cards ----------
const USAGE_RANGES = new Set(["7d", "30d", "all"]);
async function handleUsage(res, url) {
    const requested = url.searchParams.get("range") ?? "7d";
    if (!USAGE_RANGES.has(requested)) {
        sendError(res, 400, "range must be one of 7d, 30d, all");
        return;
    }
    await guard(res, async () => ({
        ok: true,
        usage: await readUsageStats(requested),
    }));
}
async function handleQuota(res) {
    ...
    await guard(res, async () => ({ ok: true, quota: await queryQuota() }));
}
/**
 ...
 */
async function handleResetCards(res, url) {
 ...
 */
async function handleResetUse(res, body) {
    const providerId = str(body, "providerId");
    ...
    try {
        await guard(res, async () => {
            const result = await useResetCard(providerId, resetType, idempotencyKey);
            ...
            if (issuedNonces.get(providerId) === nonce)
...
async function handleResetOpportunity(res, body) {
    const providerId = str(body, "providerId");
    const idempotencyKey = str(body, "idempotencyKey");
    if (!providerId || !isCodingPlanProvider(providerId)) {
        sendError(res, 403, "reset cards require an account coding-plan provider id");
...
async function handleResetHistoryRead(res, body) {
    const providerId = str(body, "providerId");
    if (!providerId || !isCodingPlanProvider(providerId)) {
        sendError(res, 403, "reset cards require an account coding-plan provider id");
        return;
 ...
 */
const resetSpendsInFlight = new Set();
/** Record the nonce a status read just issued. */
export function rememberResetNonce(providerId, nonce) {
 ...
 */
async function handleBackendRestart(res, server) {
    if (!server) {
    ...
    try {
        ...
        warn(`settings: backend restart close failed: ${error instanceof Error ? error.message : String(error)}`);
    }
 ...
 */
async function workflowGuard(res, server, fn) {
    if (!server) {
        sendError(res, 409, "workflow routes are per-bridge — use /api/instances/{id}/settings/workflows");
        return;
    }
    try {
        ...
        warn(`settings: workflow route failed: ${error instanceof Error ? error.message : String(error)}`);
        if (!res.writableEnded)
            sendError(res, 500, "internal error");
    }
}
/** Validate a scope value from a path segment or query parameter. */
function requireWorkflowScope(raw) {
...
function nonNegativeIntQuery(url, key) {
...
function requiredIntQuery(url, key) {
    const n = intQuery(url, key);
    if (n === undefined) {
...
async function handleWorkflowList(res, server, url) {
    const scopeRaw = url.searchParams.get("scope");
    await workflowGuard(res, server, async (srv) => {
        const scope = scopeRaw ? requireWorkflowScope(scopeRaw) : undefined;
        return { ok: true, ...(await listWorkflows(srv, scope)) };
    });
}
async function handleWorkflowGet(res, server, scopeRaw, name) {
    await workflowGuard(res, server, async (srv) => {
        const scope = requireWorkflowScope(scopeRaw);
        return { ok: true, ...(await getWorkflow(srv, scope, name)) };
    });
}
async function handleWorkflowMeta(res, server, scopeRaw, name, body) {
    await workflowGuard(res, server, async (srv) => {
        const scope = requireWorkflowScope(scopeRaw);
        return { ok: true, ...(await updateWorkflowMeta(srv, scope, name, body)) };
    });
}
async function handleWorkflowDelete(res, server, scopeRaw, name) {
    await workflowGuard(res, server, async (srv) => {
        const scope = requireWorkflowScope(scopeRaw);
        return { ok: true, ...(await deleteWorkflow(srv, scope, name)) };
    });
}
/**
 ...
 */
async function handleWorkflowMove(res, server, scopeRaw, name) {
    await workflowGuard(res, server, async (srv) => {
        const scope = requireWorkflowScope(scopeRaw);
        return { ok: true, ...(await moveWorkflow(srv, scope, name)) };
    });
}
async function handleWorkflowRuns(res, server, url) {
    const scopeRaw = url.searchParams.get("scope");
    ...
    await workflowGuard(res, server, async (srv) => {
        ...
        const scope = scopeRaw ? requireWorkflowScope(scopeRaw) : undefined;
        return { ok: true, ...(await listRuns(srv, { scope, name, limit })) };
    });
}
/** List a session's runs incl. the `resumable` flag (the resume button's source). */
async function handleConversationRuns(res, server, url) {
    const sessionId = url.searchParams.get("sessionId");
    await workflowGuard(res, server, async (srv) => {
        const limit = intQuery(url, "limit");
        if (!sessionId)
            throw new WorkflowApiError(400, "invalid_query", "sessionId is required");
        const zcodeSid = resolveWorkflowZcodeSid(srv, sessionId);
        return { ok: true, ...(await conversationRuns(srv, zcodeSid, limit)) };
    });
}
async function handleRunEvents(res, server, url, runId) {
    const sessionId = url.searchParams.get("sessionId");
    await workflowGuard(res, server, async (srv) => {
...
async function handleRunArtifacts(res, server, url, runId) {
    const sessionId = url.searchParams.get("sessionId");
    await workflowGuard(res, server, async (srv) => {
        if (!sessionId)
            throw new WorkflowApiError(400, "invalid_query", "sessionId is required");
        const zcodeSid = resolveWorkflowZcodeSid(srv, sessionId);
        return { ok: true, ...(await runArtifacts(srv, zcodeSid, runId)) };
    });
}
async function handleRunArtifactData(res, server, url, runId, artifactId) {
    const sessionId = url.searchParams.get("sessionId");
    await workflowGuard(res, server, async (srv) => {
        const afterSequence = nonNegativeIntQuery(url, "afterSequence");
        const limit = intQuery(url, "limit");
...
async function handleRunArtifactRead(res, server, url, runId, artifactId) {
    const sessionId = url.searchParams.get("sessionId");
    await workflowGuard(res, server, async (srv) => {
        if (!sessionId)
            throw new WorkflowApiError(400, "invalid_query", "sessionId is required");
...
async function handleRunWorkspace(res, server, url, runId) {
    const sessionId = url.searchParams.get("sessionId");
    await workflowGuard(res, server, async (srv) => {
        if (!sessionId)
            throw new WorkflowApiError(400, "invalid_query", "sessionId is required");
        const zcodeSid = resolveWorkflowZcodeSid(srv, sessionId);
        return { ok: true, ...(await runWorkspace(srv, zcodeSid, runId)) };
    });
}
async function handleRunNodeResult(res, server, url, runId, siteId, ordinalRaw) {
    const sessionId = url.searchParams.get("sessionId");
    await workflowGuard(res, server, async (srv) => {
        if (!sessionId)
            throw new WorkflowApiError(400, "invalid_query", "sessionId is required");
        ...
        return { ok: true, ...(await runNodeResult(srv, zcodeSid, runId, siteId, ordinal)) };
    });
}
/**
 ...
 */
async function handleWorkflowStart(res, server, scopeRaw, name, body) {
...
async function handleWorkflowResume(res, server, runId, body) {
    const sessionId = str(body, "sessionId");
    const name = str(body, "name");
    await workflowGuard(res, server, async (srv) => {
        if (!sessionId) {
 ...
 */
async function handleWorkflowCreatePrompt(res, server, url) {
    const scopeRaw = url.searchParams.get("scope");
    await workflowGuard(res, server, async (srv) => {
        await requireWorkflowEnabled(srv);
        const scope = scopeRaw ? requireWorkflowScope(scopeRaw) : undefined;
        return { ok: true, prompt: workflowCreatePrompt(scope) };
    });
}
// ---------- app update (ZCode desktop) ----------
const APP_UPDATE_CHANNELS = new Set(["stable", "preview"]);
/**
 ...
 */
async function handleAppUpdate(res, url) {
    const requested = url.searchParams.get("channel") ?? "stable";
    if (!APP_UPDATE_CHANNELS.has(requested)) {
        sendError(res, 400, "channel must be one of stable, preview");
        return;
    }
    await guard(res, async () => {
        const check = await checkForAppUpdate({ channel: requested });
        return {
            ok: true,
            appUpdate: {
 ...
 * field) for progress.
 */
async function handleAppUpdateInstall(res, body) {
    ...
    if (!APP_UPDATE_CHANNELS.has(channel)) {
        sendError(res, 400, "channel must be one of stable, preview");
        return;
    }
    ...
    try {
        ...
        const manifest = await fetchReleaseManifest(channel);
