 ...
 */
async function sessionRoot(server, sessionId, res) {
    if (!sessionId) {
 ...
 */
async function resolveInside(root, rel, res) {
...
async function handleList(server, url, res, head) {
 ...
 */
async function streamLines(req, res, file, line, limit) {
...
async function handleFile(server, req, url, res, head) {
    ...
    if (head) {
        ...
        const headers = {
            "Content-Type": mime,
            "Content-Disposition": contentDisposition(path.basename(file), url.searchParams.get("dl") === "1"),
        };
        if (hasByte) {
 ...
 */
export function createFileHandler(server) {
    return (req, res) => {
        ...
        if (url.pathname === "/fs/list")
            route = handleList(server, url, res, head);
        else if (url.pathname === "/fs/file")
            route = handleFile(server, req, url, res, head);
        else
            route = null;
        if (route === null) {
...
//# sourceMappingURL=file-endpoint.js.map