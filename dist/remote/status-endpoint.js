 ...
 */
export function runningZcodeSids(server) {
    const running = new Set();
    for (const turn of server.pendingTurns.values())
        running.add(turn.zcodeSid);
    return running;
}
/**
 ...
 */
export function collectStatus(server) {
    ...
    for (const acpSid of server.remoteCreatedSessions) {
        if (server.sessionSummaries.get(acpSid)?.hasActivity)
            continue;
 ...
 */
export function createStatusHandler(server) {
    return (req, res) => {
