...
export class ClientRegistry {
     ...
     */
    async notifyEach(method, build) {
        // racedNotify: per-client failure/stall isolation (see notifyAll).
        await Promise.all(this.snapshot().map((cx) => {
     ...
     * duplicate an echo.
     */
    async notifyOthers(exclude, method, params) {
        const root = exclude.connectionContext;
        const targets = this.snapshot().filter((cx) => cx.connectionContext !== root);
...
function racedNotify(cx, method, send) {
    if (stalledClients.has(cx)) {
        ...
        send.then(() => stalledClients.delete(cx), () => undefined);
    ...
    return new Promise((resolve) => {
        ...
        const timer = setTimeout(() => {
            if (!stalledClients.has(cx)) {
                ...
                warn(`broadcast: ${method} send stalled >${NOTIFY_SEND_TIMEOUT_MS}ms on one client ` +
        ...
        send.then(() => {
            ...
            warn(`broadcast: ${method} failed on one client: ` +
...
async function notifyAll(registry, method, params) {
    await Promise.all(registry.snapshot().map((cx) => racedNotify(cx, method, cx.notify(method, params))));
}
