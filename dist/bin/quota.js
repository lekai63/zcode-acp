...
import { defaultGoWindows, formatCombinedCardPlain, queryCombined, } from "../quota/combined.js";
 ...
 */
export function resolveIntervalMs(seconds) {
...
function refreshSuffix(remainingSec) {
 ...
 * `stdout.isTTY && !plain`.
 */
function renderCard(combined, provider, detail, color, refresh) {
 ...
 */
async function runWatch(intervalMs, provider, detail, color) {
 ...
 */
async function runOnce(provider, detail, color) {
    const combined = await queryCombined(provider);
    ...
    const selectedFailed = (glmSelected && glmFailed && (!goSelected || goFailed)) ||
