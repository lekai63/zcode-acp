 ...
 */
export function resolveInvocation(invokedAs, argv) {
    if (invokedAs === "zcode-acp-server")
        return { kind: "server" };
    const sub = argv[0];
    if (sub === undefined)
        return { kind: "tui", explicit: false, check: false };
    if (sub === "repl" || sub === "tui") {
...
async function main() {
    ...
    if (invocation.kind !== "help" && invocation.kind !== "unknown") {
        if (await reexecToBunIfEligible(process.argv[1] ?? "", process.argv.slice(2)))
            return;
    }
...
if (invokedDirectly) {
    main().catch((err) => {
        process.stderr.write(`zcode-acp: fatal: ${err instanceof Error ? (err.stack ?? err.message) : String(err)}\n`);
        process.exit(1);
    });
}
