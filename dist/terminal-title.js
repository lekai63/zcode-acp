 ...
 */
export function sessionTabTitle(title, cwd) {
    ...
    if (project.length > PROJECT_MAX)
        project = project.slice(0, PROJECT_MAX - 1) + "…";
    if (!project)
        project = "zcode";
 ...
 */
export function refreshTerminalTabTitle(server, acpSid, io = ttyTitleIo) {
    if (!server.marttyClientSeen)
        return false;
    ...
    if (!ok)
        log(`terminal title not written (no tty or unsupported)`);
    return ok;
}
