...
export class EventStreamListener {
    ...
    async subscribe(nextId) {
        ...
        const backoffMs = (attempt) => 500 * attempt; // 0.5s
        let resp;
        for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
            resp = await this.backend.request(nextId(), "session/subscribe", {
                sessionId: this.sid,
            ...
            log(`subscribe attempt ${attempt}/${MAX_ATTEMPTS} timed out, retrying in ${backoffMs(attempt)}ms`);
    ...
    async resubscribe(nextId) {
        ...
        if (resp.error) {
            log(`resubscribe failed (non-fatal, stall recovery degrades to polling): ${formatSubscribeError(resp)}`);
            return false;
        }
        const result = (resp.result ?? {});
...
export class TurnMonitor {
    ...
    async pollOnce() {
        const resp = await this.backend.request(this.nextId(), "session/read", 
        ...
        { sessionId: this.zcodeSid, messageLimit: 1 }, 5000);
        if (resp.error)
            return null;
        const result = (resp.result ?? {});
 ...
 */
function formatSubscribeError(resp) {
