...
export function bunVersionOk(v) {
    if (!v)
        return false;
...
export function detectBun() {
    ...
    for (const cand of bunCandidates()) {
        ...
        try {
            ...
            log(`runtime: bun at ${cand} is ${out.trim()} (< ${MIN_BUN.major}.${MIN_BUN.minor}) — staying on node`);
        }
        catch (e) {
...
export function resolveRuntime() {
    if (process.versions.bun)
        return { command: process.execPath, preArgs: ["--smol"] };
    const policy = (process.env.ZCODE_ACP_RUNTIME ?? "").trim().toLowerCase();
    if (policy === "node")
        return { command: process.execPath, preArgs: [] };
    const bun = detectBun();
    if (bun)
        return { command: bun, preArgs: ["--smol"] };
    if (policy === "bun") {
        warn("runtime: ZCODE_ACP_RUNTIME=bun but no bun >= " +
 ...
 * safe).
 */
export async function reexecToBunIfEligible(entryJs, argv) {
    if (process.versions.bun)
        return false;
    if ((process.env.ZCODE_ACP_RUNTIME ?? "").trim().toLowerCase() === "node")
        return false;
    const bun = detectBun();
    if (!bun)
        return false;
