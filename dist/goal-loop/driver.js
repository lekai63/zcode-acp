...
import { isBackendLostRequestError, preemptInFlightTurn, reloadBackendSession, runOneTurn, withPreemptLock, } from "../handlers/session.js";
...
export class GoalLoopDriver {
     ...
     */
    backendRecoveries = 0;
    static MAX_BACKEND_RECOVERIES = 3;
    constructor(server, acpSid, zcodeSid, state) {
        this.server = server;
        this.acpSid = acpSid;
     ...
     */
    static start(server, acpSid, zcodeSid, objective, opts = {}) {
        ...
        void driver
            .run(myRun)
            .catch((e) => warn(`goal-loop: driver crashed (${e instanceof Error ? e.message : String(e)})`))
            .finally(() => {
            ...
            if (server.goalLoops.get(zcodeSid) === driver)
                server.goalLoops.delete(zcodeSid);
        });
        return driver;
    }
    /**
    ...
    statusText() {
        ...
        return messages().goalStatus(s.status, s.rounds, goalMaxTurns(), done, s.tickets.length, current?.title);
    }
    /**
     ...
     */
    settleParked(r, keepText, snapshot) {
     ...
     */
    async waitForEditorTurnsIdle(myRun) {
        const deadline = Date.now() + EDITOR_TURN_WAIT_MS;
        while (Date.now() < deadline) {
            if (this.runId !== myRun)
                return;
            const busy = [...this.server.pendingTurns.values()].some((t) => t.zcodeSid === this.zcodeSid && !t.goalLoop);
            if (!busy)
                return;
            await sleep(250);
        }
    ...
    async announce(text) {
        await sendTextChunk(this.server.clients.broadcast(), this.acpSid, text, `goal-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`).catch(() => undefined);
    }
    persist() {
        writeGoalState(this.server.projectCwd(), this.zcodeSid, this.state);
    }
    async endLoop(status, reason, note) {
        ...
        this.persist();
        if (status === "stopped")
            clearGoalState(this.server.projectCwd(), this.zcodeSid);
    ...
    async run(myRun) {
        ...
        for (;;) {
            try {
                ...
                if (isBackendLostRequestError(e) &&
                    this.runId === myRun &&
                    ...
                    try {
                        ...
                        warn(`goal-loop: backend recovery failed — pausing loop (${e2 instanceof Error ? e2.message : String(e2)})`);
                        if (this.runId !== myRun)
                            return;
                        try {
                            ...
                            warn(`goal-loop: crash recovery failed (${e3 instanceof Error ? e3.message : String(e3)})`);
                        }
                        return;
                    }
    ...
    async runRounds(myRun) {
        const alive = () => this.runId === myRun && !this.server.backend?.isDead;
        ...
        if (this.state.tickets.length === 0) {
            ...
            if (res.stopReason === "cancelled") {
                if (this.consumeSandboxRestart())
                    return this.runRounds(myRun);
                return void (await this.endLoop("paused", "cancelled"));
            }
            const parsed = parseTickets(await this.lastAssistantText());
            this.state.tickets = (parsed ?? [{ title: this.state.objective, acceptance: this.state.objective }]).map((t, i) => ({
                id: `t${i + 1}`,
                title: t.title,
        ...
        while (alive()) {
            ...
            if (this.pauseFlag)
                return void (await this.endLoop("paused", "/auto pause"));
            const ticket = this.state.tickets.find((t) => t.status === "pending" || t.status === "in_progress");
            if (!ticket)
                return void (await this.endLoop("complete", "all tickets done", messages().goalComplete(this.state.rounds)));
            ...
            if (verdict?.kind === "impossible") {
                return void (await this.endLoop("impossible", verdict.why ?? "reported impossible", messages().goalImpossible(verdict.why ?? "")));
            }
            if (verdict?.kind === "met") {
                ...
                if (!v) {
                    ...
                    ticket.status = "pending";
                    this.persist();
                    return void (await this.endLoop("paused", "verification unreadable", messages().goalVerifyUnreadable(ticket.title)));
                }
                if (v.pass) {
                    ticket.status = "done";
            ...
            if (stallCount >= STALL_ROUND_LIMIT) {
                return void (await this.endLoop("paused-stall", "no tool activity", messages().goalStallPaused));
            }
            ...
            const window = modelContextWindow(cur?.providerId ?? "", cur?.modelId ?? "") ||
            ...
            await this.announce(messages().goalReport(this.state.rounds, goalMaxTurns(), ticket.title, ticket.status === "done"));
        }
    }
    ...
    async compactBoundary() {
        ...
        try {
            const hRes = await this.runGoalTurn(handoffPrompt({ objective: this.state.objective, handoffFile: hPath }));
            if (hRes.stopReason === "cancelled")
                return true;
            ...
            warn(`goal-loop: compaction failed (${e instanceof Error ? e.message : String(e)}) — continuing without`);
