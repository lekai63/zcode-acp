...
export function decomposePrompt(objective) {
    return [
        `# Objective`,
        objective,
        ``,
...
export function parseTickets(reply) {
    const lines = reply
        .split("\n")
        .map((l) => l.trim())
...
export function dispatchPrompt(opts) {
    ...
    if (handoffFile) {
        lines.push(``, `## Context was compacted since the last round`, `Read ${handoffFile} FIRST (use the Read tool) — it is the handoff from your`, `previous rounds. Treat it as your memory of prior progress; do not rely on`, `remembering anything not in it or in this prompt.`);
    }
    if (userText) {
        lines.push(``, `## User message (steering this round)`, userText);
    }
    lines.push(``, `## Rules`, `- Inspect the real current state before acting; do not rely on memory.`, `- Stay inside the scope of this ticket and the project directory.`, `- Verify your own work (run the acceptance check) before claiming done.`, ``, `## Return format (mandatory, last line of your reply)`, `VERDICT: met | not-yet | impossible`, `If not-yet: one short GAPS: line listing what remains.`, `If impossible: one short WHY: line.`);
    return lines.join("\n");
}
/**
 ...
 */
export function verifyPrompt(ticket, verifyFile) {
    return [
 ...
 * (which loops forever).
 */
export function strictVerifyPrompt(ticket, verifyFile) {
    return [
...
export function parseVerifyFile(content) {
    const text = content.trim();
    if (text === "PASS" || text.startsWith("PASS\n"))
        return { pass: true };
    const m = /^FAIL:\s*(.+)/s.exec(text);
    if (m)
        return { pass: false, reason: m[1].trim().slice(0, 400) };
    return null;
}
/**
...
export function parseVerifyReply(reply) {
    ...
    if (m)
        return { pass: false, reason: m[1].trim().slice(0, 400) };
    const negated = /\b(not|never|cannot|can't|couldn't|didn't|doesn't|won't|fails?|failed|failing|unable)\b[^\n.]*?\bpass(ed)?\b/i
        .test(text) ||
        /\bpass(ed)?\b[^\n.]*?\b(not|never|cannot)\b/i.test(text);
    if (!negated && /\bpass(ed)?\b/i.test(text))
        return { pass: true };
    return null;
}
...
export function handoffReadbackNote(handoffFile) {
