...
export async function main() {
    ...
    const inbound = Readable.toWeb(process.stdin);
    ...
    const backendDeathInterval = setInterval(() => {
        if (server.backend?.isDead)
            void shutdown("backend dead");
    }, 2000);
...
function buildAgentApp(server, allCommands) {
    ...
    return (acp
        ...
        return prompt(server, ctx.params, server.clients.broadcast(), ctx.requestId, ctx.client);
    })
        .onRequest("session/set_config_option", (ctx) => setConfigOptionHandler(server, ctx.params, server.clients.broadcast()))
        ...
        .onRequest("session/updateRuntimeModelConfig", extParams, (ctx) => updateRuntimeModelConfig(server, ctx.params))
        ...
        // mode.
        .onRequest("session/set_mode", extParams, (ctx) => setMode(server, ctx.params, server.clients.broadcast()))
 ...
 */
export function serveIdleDecision(clients, pendingTurns, lastBusy, now, idleMs) {
    if (clients > 0 || pendingTurns > 0)
        return false;
...
export async function runHeadless() {
    ...
    const allCommands = buildAllCommands();
    log(`starting ${AGENT_INFO.name} ${AGENT_INFO.version} serve (headless), project=${server.projectCwd()}`);
    let remoteHandle = null;
    let shuttingDown = false;
    const shutdown = async (reason) => {
        if (shuttingDown)
    ...
    const idleCheck = setInterval(() => {
        ...
        if (serveIdleDecision(server.clients.size, server.pendingTurns.size, lastBusy, Date.now(), SERVE_IDLE_MS)) {
