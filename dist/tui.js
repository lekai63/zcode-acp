...
export function resolveMarttyJs() {
    try {
        ...
        const pkgJson = createRequire(import.meta.url).resolve("zcode-acp-martty/package.json");
 ...
 */
export function buildTuiArgs(agentJs, interp = resolveRuntime()) {
    return [
        "--agent",
        interp.command,
 ...
 */
export async function checkTuiRuntime() {
    const marttyJs = resolveMarttyJs();
    if (!marttyJs) {
        warn("tui: martty is not installed — reinstall the zcode-acp-server package");
        return false;
    }
    const child = spawn(process.execPath, [marttyJs, "--check-runtime", ...buildTuiArgs(agentEntryJs())], { stdio: "inherit" });
...
export function resolveMarttyHome(env = process.env) {
    if (env.MARTTY_HOME)
 ...
 * Pure — exported for unit tests.
 */
export function planQuotaSeed(existingContent, assetContent) {
    if (existingContent === null)
        return { action: "write", reason: "fresh" };
    let existing;
    try {
    ...
    const asset = JSON.parse(assetContent);
    const src = existing.source ?? {};
    if (src.pluginId !== asset.source.pluginId)
        return { action: "skip", reason: "foreign" };
    const versionOf = (pkgId) => {
    ...
    if (existingContent.trim() === assetContent.trim())
 ...
 */
export function seedMarttyQuotaPlugin(env = process.env) {
    try {
        const assetPath = fileURLToPath(new URL("../assets/martty-plugins/quota-dock/plugin.json", import.meta.url));
        ...
        warn(`tui: quota dock plugin seeding failed (non-fatal): ${e instanceof Error ? e.message : String(e)}`);
        return false;
    }
}
