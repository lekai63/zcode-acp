 ...
 */
function tailLine(value) {
    const line = firstLine(value);
    if (line.length <= POPUP_TITLE_MAX)
        return line;
    let start = line.length - (POPUP_TITLE_MAX - 1);
    if (start > 0 && line.charCodeAt(start) >= 0xdc00 && line.charCodeAt(start) <= 0xdfff)
        start++;
...
export function describeToolInput(input) {
    ...
    if (typeof input !== "object")
        return headLine(String(input));
    const rec = input;
    const command = rec["command"];
    if (typeof command === "string" && command.trim())
        return headLine(command.trim());
 ...
 */
function inputPopupContent(input) {
    if (input === null || input === undefined)
        return undefined;
    let text;
    if (typeof input === "object") {
 ...
 */
export function acpPermissionResponseToZcode(acpResp, options) {
...
export function exitPlanModeToAcpPermission(params, acpSid) {
    ...
    const plan = typeof params.input === "object" && params.input !== null
        ? params.input.plan
        : undefined;
 ...
 * reason.
 */
export function buildPlanApprovalElicitationForm(params, acpSid, toolCallId) {
    const m = messages();
    const plan = typeof params.input === "object" && params.input !== null
        ? params.input.plan
        : undefined;
 ...
 */
export function parsePlanApprovalElicitationResponse(acpResp) {
    if (!acpResp || typeof acpResp !== "object") {
        return { action: "decline", reason: "invalid client response" };
    }
    const action = acpResp.action;
    if (action === "decline" || action === "cancel") {
        return { action: "decline", reason: "plan rejected" };
    }
    const content = acpResp.content ?? {};
...
export function buildAskUserAcpParams(params, acpSid, options, popupTitle) {
    return {
        options,
        sessionId: acpSid,
        toolCall: {
