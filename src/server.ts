...
export class ZcodeAcpServer {
   ...
   * a setting.
   */
  readonly paseoConnectionRoots = new Set<unknown>();
  /**
   ...
   */
  readonly subagentTrackers = new Map<string, SubagentTracker>();
  /**
   ...
   */
  async ensureSubagentTracker(zcodeSid: string): Promise<SubagentTracker> {
    const backend = await this.ensureBackend();
  ...
  async emitBackgroundTaskShutdownRecords(): Promise<void> {
    ...
    for (const tracker of this.subagentTrackers.values()) {
      try {
        ...
        warn(
          `shutdown sub-agent records failed for ${tracker.zcodeSid}: ` +
            `${e instanceof Error ? e.message : String(e)}`,
        );
      }
    }
   ...
   */
  private async notifyMethodByZcodeSid(
    zcodeSid: string,
    method: string,
    paramsFor: (alias: string) => Record<string, unknown>,
  ): Promise<boolean> {
    ...
    try {
      ...
      for (const alias of this.sessionAliases(acpSid)) {
        await enqueueSessionSend(alias, () =>
          this.clients.broadcast().notify(method, paramsFor(alias)),
        );
        sent = true;
      }
      return sent;
  ...
  async initialize(
    ...
    if ((this.clientName ?? "").toLowerCase().includes("paseo")) {
      const root = clientConnectionRoot(client);
      if (root !== undefined) this.paseoConnectionRoots.add(root);
    }
