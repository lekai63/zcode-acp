 ...
 */
export function loadActiveProvider(): ActiveProvider | undefined {
  try {
...
export function mergeEnvWithCreds(creds: ZcodeCredentials): NodeJS.ProcessEnv {
  ...
  if (provider?.baseURL) {
    try {
      ...
      log(
        `credentials: failed to write ZCode CLI provider config: ${
  ...
  if (published && !explicitBaseUrl) delete merged.ZCODE_BASE_URL;
  return merged;
}
 ...
 */
function publishProviderToCliConfig(provider: ActiveProvider): boolean {
  ...
  const providers: Record<string, unknown> = isPlainObject(config.provider)
    ? { ...config.provider }
    : {};
  const existingRaw = providers[provider.id];
  const existing: Record<string, unknown> = isPlainObject(existingRaw) ? existingRaw : {};
  const options: Record<string, unknown> = isPlainObject(existing.options)
    ? { ...existing.options }
    : {};
  ...
  const next: Record<string, unknown> = { ...config, provider: providers };
  const hasModel =
    (typeof config.model === "string" && config.model.trim().length > 0) ||
    (isPlainObject(config.model) &&
      (config.model.main !== undefined || config.model.lite !== undefined));
  if (!hasModel) next.model = `${provider.id}/${provider.modelId}`;
